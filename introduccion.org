...
Jerarquía de memoria. (Imagen: /Operating System Concepts Essentials/;
Silberschatz, Galvin, Gagne; p.11)
#+end_center
...
#+begin_center
#+attr_html: height="350"
[[./img/velocidad_memoria.png]]

Velocidad y gestor de los principales niveles de memoria. (Imagen:
...
eventos son manejados a través de las /interrupciones/.
...
Los eventos pueden ser, como mencionamos, indicadors de que hay algún
...
Cada sistema operativo /expone/ una /interfaz al programador/ (API)
que incluye todas las operaciones que se le pueden solicitar, cómo
hacerlo y con qué argumentos.

Cada familia de sistemas operativos implementa un API distinto, y éste
es el que distingue principalmente a uno de otro. Por ejemplo, los
sistemas Windows 95 en adelante implementan Win32, Win16
(compatibilidad con Windows previos) y MSDOS; MacOS implementa Cocoa
(aplicaciones MacOS X) y Carbon (compatibilidad con aplicaciones de
MacOS previos), y Linux y los *BSDs, POSIX (el estándar que define a
Unix). El caso de MacOS X es interesante, porque también implementa
POSIX, ofreciendo la /semántica/ de dos sistemas muy distintos entre
sí.
