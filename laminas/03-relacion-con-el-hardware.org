...
- Todas las computadoras que empleamos hoy en día son, para fines
  prácticos, sistemas von Neumann

** Implicaciones de /von Neumann/
\centering La arquitectura /no considera/ la existencia de almacenamiento /persistente/
- Dentro del procesador hay algunas (¡pocas!) localidades para
...
- Memoria super-rápida (hoy en día, sub-nanosegundo), ubicada /dentro/
  del procesador
- Manejada /por referencia directa/ (por /su nombre/), no por dirección
...
#+caption: Wikipedia: MOS Technology 6502
[[../img/6502.jpg]]

#+attr_latex: :width \columnwidth
#+caption: Wikipedia: Zilog Z80
[[../img/z80.jpg]]
#+latex: \end{column}
#+latex: \end{columns}
...
- Típicamente $\ge 32$ registros /largos/ (32, 64 bits) de propósito
  general
  - Mas algunos de propósito específico

** Tipos de almacenamiento
#+BEGIN_CENTER
Todos nuestros /datos e instrucciones/ deben almacenarse en memoria, pero...
#+END_CENTER
- La memoria /muy/ rápida es muy cara
- La memoria barata es muy lenta (/hasta 1000 veces/ más lenta que el procesador)
#+latex: \begin{columns}
#+latex: \begin{column}{0.3\textwidth}
#+attr_latex: :width \textwidth
#+caption: Generada en raphael.app
[[../img/write_to_ram.png]]
#+latex: \end{column}
#+latex: \begin{column}{0.7\textwidth}
...
- Los CPUs son > 1000 veces más rápidos que en 1978 (cuando John Backus
  hizo esta observación)
- La velocidad de acceso a la memoria ha mejorado... /marginalmente/
#+latex: \end{column}
#+latex: \begin{column}{0.4\textwidth}
#+attr_latex: :width \textwidth
#+caption: Generada en raphael.app
[[../img/vn_bottleneck.png]]
#+latex: \end{column}\end{columns}
...
- Acomodo /muy frecuente/ en sistemas x86 de hace algunos años[fn:: Desde
  del 2010, las funciones, primero del /Northbridge/ y luego del
  /Southbridge/, han sido absorbidas por el CPU mismo]:
...
#+attr_latex: :width \textwidth
#+caption: Diagrama de la comunicación entre componentes de un sistema de cómputo basado en el chipset Intel 875 (Pentium 4, 2003)
...
** Ejemplo: Raspberry Pi familias 1–3
#+attr_latex: :width \textwidth
...
#+latex: \end{column} \begin{column}{0.3\textwidth}
#+attr_latex: :width \textwidth
#+caption: Generada en perchance.org
[[../img/exception.jpg]]
#+latex: \end{column} \end{columns}
...
** Llamadas al sistema
- Análogas/complementarias a las interrupciones
...
#+BEGIN_CENTER
*Problema:* Cuando un proceso está /limitado por entrada-salida/, la
transferencia de información se vuelve cuello de botella
#+END_CENTER
#+latex: \begin{columns} \begin{column}{0.7\textwidth}
...
\pause \centering
*Ojo:* Es frecuente ver a estos referidos como tipos de /multitarea/

\small (en buena medida, porque el /multiprocesamiento/ no era aún común)
...
- Hace apenas 20 años, muchos sistemas operativos no detectaban
...
#+latex: \end{column}\begin{column}{0.3\textwidth}
#+BEGIN_CENTER
Y a cinco décadas, la Ley de Moore se sigue cumpliendo...
#+END_CENTER
#+latex: \end{column}\end{columns}
...
- El reloj de los procesadores no sólo no ha crecido, sino que en
  ...
  - Apenas 15 años más tarde volvimos a esas velocidades
...
#+end_export
#+attr_latex: :width \textwidth
#+caption: Motherboard NUMA HP Z820 (Wikipedia)
[[../img/motherboard_numa.jpg]]
...
- Parecerían un punto medio entre SMP y el /cómputo distribuído/
  - /¿Cómputo distribuído fuertemente acoplado?/

    \pause Hmmm... No exactamente
...
\pause
#+BEGIN_CENTER
¿Alguien puede mencionar el ejemplo de alguna malla?
#+END_CENTER

** Cómputo en la nube
- Caso específico, término /de moda/
...
#+caption: Cloud computing (de /linuxaria.com/)
[[../img/cloud.jpg]]
#+latex: } \only<2->{ \centering \Large
La nube no existe... 
#+latex: \vfill \,} \only<3>{ \centering \Large
Es sólo la computadora de otra persona.
#+latex: \vfill \,}
...
- La /porción paralelizable/ puede repartirse entre varios
  procesadores, pero siempre hay una importante proporción no
  paralelizable
  - Inicialización
