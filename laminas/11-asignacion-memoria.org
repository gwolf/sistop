...
** El /intercambio/ (/swap/) en la memoria
- El SO puede /comprometer/ (otorgar) más memoria de la que tiene disponible
- Cuando inicia un programa que /no cabe en memoria/, el SO puede elegir
...
- En Intel 8086, 6 registros de segmento, mas un /desplazamiento/,
  dan el espacio de direccionamiento de 20 bits (1MB)
...
- Este esquema se mantuvo hasta la arquitectura x86 a 32 bits
  - Al aparecer /x86-64/, se abandonó (en gran medida) a favor de la
    /paginación/.

** Ejemplo de uso en /x86-32/
...
- No se pueden cargar valores directamente a los registros de segmento; se hace
  pasando por =AX=
- El registro de datos =DS= apunta a la dirección base de memoria =0x00400000=
- Se carga en el registro =EBX= el /desplazamiento/ requerido dentro del
  segmento de datos
- Se /linealiza/ la dirección requerida $\rightarrow$ combina el contenido de
  =DS= con el desplazamiento =EBX=: 0x00401234

#+latex: \end{column} \end{columns}
\scriptsize *Ojo: /Un poquito/ sobresimplificado*, para no entrar en detalles de
la indirección mediante Global Descriptor Table y otros, que incluye límites y
permisos para cada segmento...
...
- La memoria está dividida en una serie de /páginas/, todas ellas del
  ...
  - Típicamente 4K ($2^{12}$)
  - Máximos hoy, 16MB ($2^{24}$) (aunque... /nadie usa tanto/ — Salvo
    en casos especiales)
...
- Fragmentar la memoria física en páginas muy pequeñas reduce la
  ...
  - U obliga a que la transferencia se haga a marcos dentro del
    espacio de memoria del kernel, obligando a una copia RAM-a-RAM
    posterior
...
** ¿Cuánto ocuparía en una computadora real +actual+ viejita?
#+BEGIN_CENTER
Computadora sencilla +actual+ de hace ≈15 años: Páginas de 4096 bytes
($2^{12}$), espacio de direccionamiento de 32 bits...
#+END_CENTER
\pause
...
\pause
- ¿Y qué caché me /aguanta/ 5MB?
** ¿Cuánto ocuparía en una computadora real actual?
#+BEGIN_CENTER
... ¿Quién se aventura a sacar números para memoria de 48 bits?
...
- Una estrategia: Almacenar /la propia tabla de páginas/ en memoria
  ...
  - Emplear /sólo dos/ registros especiales:
    ...
    - PTLR :: Registro Límite de Tabla de Páginas (/Page Table Limit
              Register/)
  - *¿Y por qué el PTLR?* La mayor parte de los procesos /nunca requerirá/ del
    direccionarlo completo
    - Para no tener un mapa completo mayormente vacío, PTLR indica la extensión
      máxima empleada
...
- Como vimos, una sobrecarga de 5MB por proceso en 32 bits. Y de
  nuevo... ¿en 48 bits?
- Tendríamos $2^{26}$ ($\approx$ 64 millones) entradas con páginas de 4MB
  ($2^{22}$)
- $2^{48} / 2^{22} = 2^{26}$ entradas, cada una ocupando 26 bits
  para la página y 26 bits para el marco
- $2^{26} \times (26 + 26)$ \rarrow \approx 3.5GB /por proceso/!
...
- El TLB guarda la resolución para los /20 bits/ de ambas tablas
  - O los 38 bits correspondientes en una arquitectura de 48 bits
