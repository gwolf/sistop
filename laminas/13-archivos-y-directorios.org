...
#+begin_src dot :exports results :file ltxpng/directorio_DG.png
digraph G {
  root -> home;
...
#+begin_src dot :exports results :file ltxpng/directorio_DG_completo.png
digraph G {
  root -> root [label="."];
  root -> root [label=".."];
...
- Primeras implementaciones, sobre un protocolo de red propio
  (NetBEUI); migrado gradualmente para operar sobre TCP/IP
...
- Asume modificaciones a /archivos completos/
  ...
  - Popularizó el concepto de /maildir/: Cada correo es guardado como
    un archivo independiente en un directorio (para facilitar las
    escrituras), en vez del archivo /mbox/ empleado anteriormente
