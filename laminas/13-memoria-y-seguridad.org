...
- En un sistema de cómputo, no podemos asumir /la mejor intención/
  - Un sistema puede tener distintos usuarios, el sistema debe proteger a
...
- La memoria es el principal /vector de ataque/
  ...
  - ...Y el procesador *no puede /entenderla/* — únicamente puede */obedecerla/*

** ¿No basta con configurar correctamente los permisos?
- La memoria segmentada y paginada ya incluyen los conceptos de
  permisos
- Desde hace muchos años, los segmentos /de texto/ (o las páginas
  correspondientes) no admiten escritura
...
#+end_src
#+begin_center
Parecería que el sistema /atrapó/ al error exitosamente. \pause ¿O no?
#+end_center
...
- En este caso, la longitud /mágica/ es de 272 bytes (256 del buffer +
  8 de =RET= + 8 de =SFP=
- ¿Qué pasa si sobreescribimos =RET= con una dirección /válida/?

** Brincando a otro punto del programa
- Es necesario comprender cómo el compilador /transforma/ el aparente
...
- Si hacemos que el flujo /brinque/ a inmediatamente después del
  =bne=, libramos la verificación
...
- Técnica de explotación conocida como /Programación orientada a
  retornos/ (/Return-Oriented Programming/)
