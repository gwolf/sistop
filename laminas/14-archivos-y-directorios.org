...
** Tipo de datos abstracto (ADT, ¿programación orientada a objetos?)
...
- O /subsistemas particulares/
  - Como pronto veremos, los /subdirectorios/ en nuestros sistemas de
    archivos son simplemente archivos de tipo especial
  - Gestionados directa y exclusivamente por el SO (e inaccesibles
...
- Caso especial del acceso aleatorio
- El programa de usuario /no usa/ la interfaz /tradicional/ de cinta,
  sino que pide la inclusión de determinado archivo /dentro de su
  espacio de memoria/
#+BEGIN_SRC python
from os import path
...
#+END_SRC
#+BEGIN_CENTER
Más detalles: [[https://manpages.debian.org/jessie/manpages-dev/mmap.2.en.html][man 2 mmap]]
#+END_CENTER
...
- En realidad, es una abstracción que nos da el sistema operativo —
  ...
  - Históricamente (y por más de 50 años), 512 bytes
  - Hoy en día (2009--), 4K (/Formato Avanzado/)
...
- Conviene diseñar estructuras de datos /alineadas al tamaño del bloque/
  - Un registro de 400 bytes en un dispositivo con bloques de 512
    bytes /casi siempre/ (>75%) requerirá la transferencia de dos
    bloques
...
- Hoy en día, eso significa hablar de /directorios/
  - Aunque existen otras formas de organización
...
- Cambia también el /caracter separador/ — Indica el fin de un
  ...
  - MacOS histórico ($\le 9.x$) :: Dos puntos (=:=)
...
- Por ejemplo, el sistema de /almacenamiento en la nube/ Amazon S3
  ...
  - De cierto modo, similar a los contenidos de =.git/objects=:
    Requieren saber /qué objeto queremos encontrar/. No es su papel
    proveer /organización de archivos/.
...
#+attr_latex: :width 0.45\textwidth
#+caption: Directorio como un /grafo dirigido/ (ya no /acíclico/), mostrando en gris los /enlaces ocultos/ =.= y =..=
...
#+latex: \small
- *Unix:* ~$ ln old.txt new.txt~
- *Windows (~cmd~):* ~C:\> mklink /h old.txt new.txt~
- *Windows (Powershell):* ~C:\> New-Item -ItemType HardLink -Path new.txt -Target old.txt~
...
- Es un archivo /especial/ que incluye el nombre de otro objeto
  (¿archivo?) destino
  - Cuando es abierto, /el sistema operativo mismo/ sigue la
    indirección
  - Es transparente al programa en espacio de usuario
...
#+latex: \small
- *Unix:* ~$ ln -s old.txt new.txt~
- *Windows (~cmd~):* ~C:\> mklink old.txt new.txt~
- *Windows (Powershell):* ~C:\> New-Item -ItemType SymbolicLink -Path new.txt -Target old.txt~


** /No es lo mismo/: Accesos directos
- Ambos tipos de ligas duras existen tanto en Unix como en Windows
  - Aunque en Windows /casi/ nunca son empleadas
- Son una herramienta muy útil y poderosa — En Windows 95 (FAT16/VFAT) se
  introdujo el /acceso directo/ buscando implementar /parte/ de su
  funcionalidad
...
- ¿Principal diferencia? No se /siguen/ automáticamente
  ...
  - Un /acceso directo/ puede abrirse como un archivo; leer una /liga
