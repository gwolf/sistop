...
- iOS :: El sistema operativo de Apple, y diseñado exclusivamente para
         ...
         universalización en el uso de dispositivos móviles.
...
- Windows Phone :: Microsoft ofrece una versión de su sistema
                   operativo, compilado para procesadores ARM. Este
                   sistema operativo se ha mantenido como muy poco
                   popular, en claro contraste con su dominación en el
                   cómputo /tradicional/; el principal fabricante que
                   vende equipos con /Windows Phone/ es Nokia (que,
                   después de haber sido la compañía líder en
                   telefonía, fue adquirida por Microsoft mismo).
...
# Symbian EKA2: 
#   supports sufficiently fast real-time response to build a single-core
#   phone around it—that is, a phone in which a single processor core
#   executes both the user applications and the signalling stack.

# - Movilidad y GPS
# - Reconfigurabilidad de las redes
# - No hay swap; FS basado en estado sólido
# - CPU y OS orientados al bajo consumo (uso de batería)
