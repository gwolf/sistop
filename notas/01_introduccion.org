...
systems that will fail]]/), adecuar un sistema a un entorno
...
#+latex: \textsc{led}s.
#+html: <span class="smallcaps">led</span>s.
...
#+latex: \textsc{basic},
#+html: <span class="smallcaps">basic</span>,
 diseñado para usuarios novatos en los sesenta, y para permitir a
...
oficina más que al /hobby/, a principios de los ochenta (particularmente
representadas por la
#+latex: \textsc{ibm pc},
#+html: <span class="smallcaps">ibm pc</span>,
 1981), sus sistemas operativos se
...
ejecución/. El papel principal del sistema operativo ante el usuario era
administrar los archivos de las diversas aplicaciones mediante
una sencilla interfaz de línea de comando, y lanzar las
aplicaciones que el usuario seleccionaba.

La
#+latex: \textsc{pc} de \textsc{ibm}
#+html: <span class="smallcaps">pc</span> de <span class="smallcaps">ibm</span>
fue la primer arquitectura de computadoras personales en
...
eventualmente capturaron casi 100% del
mercado. Prácticamente todas las computadoras de escritorio y
portátiles en el mercado hoy derivan de la arquitectura de la
#+latex: \textsc{ibm pc}.
#+html: <span class="smallcaps">ibm pc</span>.
...
Ante las aplicaciones, el sistema operativo (
#+latex: \textsc{pc-dos},
#+html: <span class="smallcaps">pc-dos</span>,
en las versiones
distribuidas directamente por
#+latex: \textsc{ibm},
#+html: <span class="smallcaps">ibm</span>,
o el que se popularizó más,
#+latex: \textsc{ms-dos},
#+html: <span class="smallcaps">ms-dos</span>,
...
hardware, arrancando desde el
#+latex: \textsc{bios}
#+html: <span class="smallcaps">bios</span>
y sin emplear el sistema operativo.
...
#+html: <em><span class="smallcaps">gui</span>s</em>)
basadas en el paradigma
#+latex: \textsc{wimp}
#+html: <span class="smallcaps">wimp</span>
(/Windows, Icons,
...
la primer interfaz de
#+latex: Mac\textsc{os}
#+html: Mac<span class="smallcaps">os</span>
...
/niveles de ejecución/ ni /unidad de administración de memoria/
#+latex: \textsc{(mmu}),
#+html: <span class="smallcaps">(mmu</span>),
...
#+latex: Mac\textsc{os}
#+html: Mac<span class="smallcaps">os</span>
 y Microsoft Windows) no implementaban multitarea real, sí
hubo desde el principio sistemas como la Commodore Amiga o la Atari
#+latex: \textsc{st}
#+html: <span class="smallcaps">st</span>

que hacían un multitasking /apropiativo/ verdadero.
...
Conforme fueron apareciendo los
#+latex: \textsc{cpu}
#+html: <span class="smallcaps">cpu</span>
...
principales: la derivada de
#+latex: \textsc{ibm}
#+html: <span class="smallcaps">ibm</span>
y la derivada de la Apple Macintosh.

Los sistemas operativos primarios para ambas plataformas fueron
respondiendo a las nuevas características del hardware: en las
#+latex: \textsc{ibm},
#+html: <span class="smallcaps">ibm</span>,
la presencia de Microsoft Windows (originalmente un /entorno operativo/
desde su primera edición en 1985, evolucionando hacia un sistema
operativo completo ejecutando sobre una base de
#+latex: \textsc{ms-dos}
#+html: <span class="smallcaps">ms-dos</span>
en 1995) se fue
...
operativo completo y ya no como una aplicación dependiente de
#+latex: \textsc{ms-dos}:
#+html: <span class="smallcaps">ms-dos</span>
el núcleo de nueva tecnología (Windows
#+latex: \textsc{nt}),
#+html: <span class="smallcaps">nt</span>),
que, sin romper
compatibilidad con los
#+latex: \textsc{api}s
#+html: <span class="smallcaps">api</span>s
...
#+html: Mac<span class="smallcaps">os x</span>
es un sistema basado en un núcleo Unix
#+latex: \textsc{bsd},
#+html: <span class="smallcaps">bsd</span>,
sobre el /microkernel/ Mach.
...
#+html: *<span class="smallcaps">bsd</span> (Free<span class="smallcaps">bsd</span, Net<span class="smallcaps">bsd</span>, Open<span class="smallcaps">bsd</span>).
Estos sistemas implementaron, colaborativamente y a
escala mundial, software compatible tanto con las
#+latex: \textsc{pc}
#+html: <span class="smallcaps">pc</span>
como con el que
...
Al día de hoy, la arquitectura derivada de Intel (y la
#+latex: \textsc{pc})
#+html: <span class="smallcaps">pc</span>)
es el claro
...
norma es la arquitectura
#+latex: \textsc{arm},
#+html: <span class="smallcaps">arm</span>,
...
interfaz primaria de Windows 8, o los entornos
#+latex: \textsc{gnome}
#+html: <span class="smallcaps">gnome</span>
y Unity de Linux)
...
#+latex: \textsc{ibm}
#+html: <span class="smallcaps">ibm</span>
5100, de 25kg de peso y con una pantalla
...
fue, a fin de cuentas, iniciada por el
#+latex: \textsc{cpu}
#+html: <span class="smallcaps">cpu</span>
Intel 4004, diseñado
expresamente para las calculadoras Busicom. Durante esa época
nacieron las calculadoras portátiles. Éstas comenzaron implementando
...
operaciones trigonométricas. En 1974, Hewlett-Packard lanzó al mercado la
#+latex: \textsc{hp-65},
#+html: <span class="smallcaps">hp-65</span>,
...
transferencias (como se verá en la secc. \ref{HW_SYSCALLS} y en el
cap. \ref{FS}), el programador tenía que avanzar y transferir
...
que fuera el dominante del mercado de celulares
...
Digital Assistant/,
#+latex: \textsc{pda}).
#+html: <span class="smallcaps">pda</span>).
Hubo diferentes hitos durante la década de
los noventa, aunque destaca particularmente la plataforma /Palm/. Esta fue
la primera plataforma con éxito al incorporar una interfaz usuario táctil
...
popular teléfono celular con la del
#+latex: \textsc{pda}.
#+html: <span class="smallcaps">pda</span>.
Ya desde 1996 se
...
inteligentes (e incluso, llevada más al extremo): Es posible encontrar antecedentes
...
#+latex: \textsc{xx},
#+html: <span class="smallcaps">xx</span>,
y varias implementaciones funcionales desde
inicios de la década de los noventa. Sin embargo, las tabletas parecieron
...
- iOS :: El sistema operativo de Apple, y diseñado exclusivamente para
	 ...
	 #+html: Mac<span class="smallcaps">OS X</span>, i<span class="smallcaps">os</span>
	 está basado en el núcleo /Darwin/,
         derivado de
	 #+latex: Free\textsc{bsd},
	 #+html: Free<span class="smallcaps">bsd</span>,
...
- Windows Phone :: Microsoft ofrece una versión de su sistema
                   operativo, compatible a nivel
		   #+latex: \textsc{api}
		   #+html: <span class="smallcaps">api</span>
		   con el Windows de escritorio, pero compilado para
                   procesador
                   #+latex: \textsc{arm}.
		   #+html: <span class="smallcaps">arm</span>.
		   Este sistema operativo no ha logrado
...
- Firefox OS :: La fundación Mozilla, responsable del navegador
                ...
                siguiendo completamente los estándares y lenguajes
                para uso en la Web. Esta plataforma hace una apuesta
...
los sistemas operativos ampliamente utilizado, fue
#+latex: \textsc{apm}
#+html: <span class="smallcaps">apm</span>
(/Advanced
...
fue reemplazado por
#+latex: \textsc{acpi}
#+html: <span class="smallcaps">acpi</span>
(/Advanced Configuration and
Power Interface/, /Interfaz Avanzada de Configuración y Energía/); la
principal diferencia entre ambas es que, mientras que bajo
#+latex: \textsc{apm}
#+html: <span class="smallcaps">apm</span>
la implementación de los diferentes niveles de energía se implementaban
en el /firmware/ de la computadora o cada uno de sus dispositivos, en
#+latex: \textsc{acpi}
#+html: <span class="smallcaps">acpi</span>
la responsabilidad recae en el sistema operativo; esto brinda
...
informática/. Puede verse el impacto de este concepto en prácticamente
...
seguridad. Algunas consideraciones ejemplo podrían ser:

- Abstracción :: El sistema operativo debe asegurarse no sólo de
                 proveer las abstracciones necesarias, sino que
                 también debe asegurarse de que ninguno de sus
                 usuarios pueda /evadir/ dichas abstracciones. Por
                 ejemplo, el que un usuario tenga derecho de modificar
                 un archivo que esté alojado en determinada unidad de
                 disco, ese usuario no debe poder escribir
...
- Aislamiento :: Si el sistema operativo ofrece separación entre los
                 ...
                 sus acciones, el sistema /penalice/ a otros usuarios
...
vista antagonistas: El la /protección/, que busca definir y proteger
...
puntos es muy distinto, comprender ambos puntos de vista resulta no
únicamente legítimos sino necesario para una formación profesional
completa.

Todas las áreas que aborda la presente obra tienen aspectos que pueden
analizarse desde el punto de vista de la seguridad informática, y más
que dedicar un capítulo en particular a este tema, la apuesta es por
abordar a la seguridad de forma transversal.
...
/malware/) consiste en programas diseñados para /aprovechar/ a dichas
...
su presencia generalizada más de 40 años después de haberse descrito,
...
concienizando a los desarrolladores de las prácticas responsables de
programación.]

Cabe mencionar que una gran cantidad de código malicioso atacan a una
...
y dado que sus diferencias radican particularmente en su diferente
comportamiento /ante el usuario/ o /como programa en ejecución/ (y se
...
- Sistemas con concepciones híbridas :: No se puede  hablar de
     ...
     #+latex: \textsc{fuse},
     #+html: <span class="smallcaps">fuse</span>,
     en Linux).
...
1. ¿En qué se diferencía la multitarea apropiativa de la cooperativa?
...
2. Un sistema operativo ofrece una serie de recursos o características
   ...
   conceptos? ¿Cuál no corresponde a una función del sistema operativo?
...
5. Se presentó que un sistema microkernel se basa en la simplificación
...
6. De los sistemas operativos ampliamente utilizados que conozca,
   ¿cuáles son microkernel? ¿cuáles son monolíticos?

7. Si bien los sistemas operativos empleados para dispositivos móviles
   ...
   Esto es, ¿qué tanto ha influído la popularización de los
...
- /Making EPERM friendlier/
  ...
  limitantes de la semántica POSIX como la falta de granularidad en el
