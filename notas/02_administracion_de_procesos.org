...
*** Problema productor-consumidor
  ...
  =add()= y =get()= intentan manipular sus estructuras al mismo
...
*** Problema lectores-escritores
  ...
  #+begin_src python
  import threading
  global lectores, mutex, cuarto_vacio
  lectores = 0
  mutex = threading.Semaphore(1)
  ...
  #+begin_src python
  import threading
  global lectores, mutex, cuarto_vacio, torniquete
  lectores = 0
  mutex = threading.Semaphore(1)
  cuarto_vacio = threading.Semaphore(1)
  ...
  def escritor():
    global cuarto_vacio, torniquete
    torniquete.acquire()
    cuarto_vacio.acquire()
    escribe()
    cuarto_vacio.release()
  ...
  escritor suelte este mutex exterior.

  Como nota adicional: Este ejemplo ilustra cómo un mismo objeto puede
  ser empleado de dos maneras distintas: Mientras =torniquete= es para
  =lector= un torniquete, para =escritor= es una mutex.
...
Al haber tres distintos ingredientes, tiene sentido que empleemos
tres distintos semáforos, para señalizar a los fumadores respecto a
cada uno de los ingredientes. Un primer acercamiento podría ser:
  ...
  #+BEGIN_SRC python
    ...
    def agente():
      while True:
        ...
        mis_semaf = semaforos.values()
        mis_semaf.remove(random.choice(mis_semaf))
        for i in mis_semaf:
          i.release()
...
- Inhabilitación de interrupciones ::
     ...
     siguieran ejecutando en los otros procesadores del sistema. Y si
     bien también sería posible, detener a los demás procesadores
     representa un costo demasiado alto, mucho más que el de los
...
- Instrucciones atómicas ::
     ...
     #+BEGIN_SRC C
     while (test_and_set_lock(candado)) {}
     seccion_critica();
     free_lock(candado);
     #+END_SRC
     ...
     Un problema con esta solución es que fomentaría la /espera
     activa/, desperdiciando tiempo de procesador cada vez que un
     proceso está esperando al candado. Seguiría siendo necesaria la
     existencia de algoritmos para evitar situaciones de inanición.
