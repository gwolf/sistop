...
requiere en primer término de la transferencia a memoria, y sólamente
...
dentro de cada uno de los núcleos de cada uno de los *cpu*. Las
...
pérdida notable de información, no ateneder el paquete de red sí.
...
- Administrar el hardware manejador de interrupciones :: Esto incluye
     el enmascarado y desenmascarado de las interrupciones, configurar
     y asignar interrupciones a cada dispositivo, notificar al
     manejador cuando la interrupción ya ha sido atendida, etc.
- Abstraer las interrupciones :: El sistema operativo oculta a los
     programas de usuario la existencia de interrupciones de hardware
     ya que éstas son dependientes de la arquitectura del
     procesador. En cambio el sistema operativo lo comunica de una
     forma unificada a través de distintos mecanismos, por ejemplo
...
- Punto de entrada al sistema operativo :: Como se verá más adelante
     en la sección \ref{HW_SYSCALLS}, muchos procesadores y sistemas
...
gráficas las cuales podían representar imágenes junto con texto. Con
...
ellos por su nombre en inglés: /buses/). Al nivel más
...
controladores de /acceso directo a memoria/ (*dma*) en uno o más de
sus subsistemas.
...
sistema, y sus lenguajes/bibliotecas implementan distintos
#+latex: \textsc{api}s.
#+html: <b>api</b>s.
Esto es el que
...
la familia *.net* (ver sección \ref{VIRT_arq_inexistentes}), exponen
...
serán cubiertos en la sección \ref{DIR_arch_especiales}.] que existen
...
- Dispositivos de bloques :: Presentan una interfaz
...
Además de esto, en los sistemas Unix existen los /sistemas de archivos
...
son alcanzables únicamente a través de llamadas a sistema, haciendo
...
/multiprogramación/, mucho más acorde a los equipos que se emplean día
...
una computadora multiprocesada y el cómputo distribuído.
...
- El Módulo de Plataforma Confiable (TPM o /SecureBoot/) ::
  ...
  Tras muchos años de encontrar gran resistencia por parte de la
  ...
  Como lectura para este tema resultan referencia obligada los varios
  ...
  Otro artículo interesante al respecto es el de, de Jonathan Corbet
