...
3. ¿Cuál es el máximo de información (en bytes) que puede transferir a
   memoria en un segundo un disco con un tiempo de acceso de 15ms y
   una tasa de transferencia de 80MB/s?
...
5. Si el procesador de un sistema corre a 1GHz, la latencia de la
   memoria es de 130ns, y el bus de memoria puede sostener un ancho de
   banda de $3~000$ MB/s, asumiendo que no hay ningún otro proceso en
   ejecución que entorpezca el proceso, ¿cuántos ciclos de reloj tiene
   que esperar el procesador al solicitar una palabra (64 bits)?
