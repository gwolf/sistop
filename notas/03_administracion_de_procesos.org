...
/bloque de control de proceso/ (*pcb* - /Process Control Block/). El 
...
- Información de planificación (scheduling) :: La prioridad del
     ...
     profundizará el tema en el capítulo \ref{PLAN}.
...
capaces de ejecutar una máquina virtual (ver sección
...
Para profundizar más en este tema, se recomienda el libro «[[http://greenteapress.com/semaphores/][The little
book of semaphores]]» de Allen Downey (2008).
...
- Esto parece muy específico ::
  ...
  Si bien este análisis parece muy específico es fácil ver que la
...
- ¿Vale la pena preocuparse? ::
  ...
  A modo de ilustración de la gravedad del problema, estos son algunos
  valores para el resultado final de la variable =cuenta= cuando se
  ejecuta el programa anterior en Pascal-FC[fn::
  http://www-users.cs.york.ac.uk/burns/pf.html]: 25 29 31 20 21 26 27
  18 31 35. Notesé que incluso uno de los valores es menor que /20/
...
- Condición de carrera :: (En inglés, /Race condition/) Categoría de errores de
...
- Recurso compartido :: Un recurso que puede ser accedido desde más de
     ...
     (como =cuenta= en el jardín ornamental),
...
En el problema hay claramente un /recurso compartido/ que es la =cuenta=, así la
...
presentan a continuación, pues, los sucesivos /intentos/.
...
- Intento 4: Manejar la bandera con instrucciones atómicas ::
  ...
  waiting/ en inglés) y es una situación que se desea evitar.
...
- Espera activa :: La solución presentada mantiene todavía el problema
		   ...
		   Para mayores detalles acerca de las razones, ventajas y desventajas
                   ...
                   referirse a [[http://www.cs.fsu.edu/~baker/devices/notes/spinlock.html][Spin Locks & Other Forms of
                   Mutual Exclusion]] (Theodore P. Baker 2010)
...
#+caption: Sincronización: la exclusión de las /secciones críticas/ entre varios procesos se protegen por medio de regiones de /exclusión mutua/. La figura ilustra a varios procesos que requieren de una misma sección crítica; las flechas punteadas representan el tiempo que un proceso está a la espera de que otro libere el paso para dicha sección, y las punteadas verticales indican la transferencia del /candado/.
...
de un candado explícito — se podría leer la línea 5 como /exclusión
...
- Apagador :: Cuando se tiene una situación de /exclusión categórica/
	      ...
	      continuación para el problema de los lectores y los
...
- Cola :: Se emplea una cola cuando se tienen dos /clases de/
...
- Espera :: Se le indica una condición y un mutex. El mutex tiene que
            haber sido ya adquirido. Esta operación /libera/ al mutex,
            y se bloquea hasta recibir una /notificación/ de otro hilo
            o proceso. Una vez que la notificación es recibida, y
            antes de devolver la ejecución al hilo, /re-adquiere/ el
            mutex.
...
*** Planteamiento
     ...
     muchos procesos /lectores/, pero si algún proceso está
...
*** Planteamiento
     ...
     - Sólo un filósofo puede sostener determinado palito a la vez, esto es
...
abstractos/ (en inglés, /Abstract Data Types/, *adt*), /clases/ de
/objetos/, y /exponen/ una serie de /métodos públicos/, además de
poseer /métodos privados/ que emplean internamente.
...
[[http://www.cs.fsu.edu/~baker/realtime/restricted/notes/philos.html][ejemplo de Ted Baker]], sobre la solución propuesta por Tanenbaum]. Esto
...
antes descripta se resolvería de la siguiente manera:
...
/Bloqueo inminente/, en color naranja (en $3 \le t_A \le 7$ y $2 \le
...
su reclamo máximo/ (llamese =claim()=) al sistema: el número máximo
...
hacerlo, porque incluso una vez terminando /A/, se tendrían sólo 6
...
5. Resuelva el siguiente planteamiento, empleando múltiples hilos que
   ...
   no pueden comunicarse entre sí (o con el cliente) más que a través
   de primitivas de sincronización. Tenemos los siguientes procesos y
   roles:
   ...
   - 1 Cajero :: Recibe la órden del cliente. Cobra el dinero /antes/
                 de que comience a procesarse la órden.

   - 3 Cocineros :: Saca la carne del refrigerador. Pone a la carne
                    sobre la plancha. Le da la vuelta a cada carne
                    para que quede bien cocida. Pone a la carne sobre
                    una base de pan. Sólo puede sacar la carne de la
                    plancha si ya tiene una base de pan lista.

		    Puede haber más de un cocinero, pero la estufa
                    sólo tiene espacio para dos carnes. ¡Recuerda
                    evitar que los cocineros pongan más de las que
                    caben!

   - 1 Armador :: Mantiene suficientes panes (y no demasiados, porque
                  se secan) aderezados y listos para que el cocinero
                  deposite la carne. Una vez que un pan con carne está
                  listo, le agrega los ingredientes que hagan falta,
                  lo cierra y entrega al cliente.

   Implemente en pseudocódigo la solución para este
   sistema. Identifique qué patrones de sincronización fueron
   empleados.
...
- Sistemas de arranque modernos en sistemas tipo Unix ::
  ...
  *bsd*). Pero la realidad del cómputo ha cambiado con el
  ...
  referenciarefiero?  ¿Por qué los esquemas tradicionales ya no son
...
- Problemas adicionales de sincronización ::
  ...
  De todo el libro, es en este capítulo en que más se trata
  ...
  El libro libremente redistribuíble [[http://greenteapress.com/semaphores][The little book of semaphores]]
  (Allan Downey, 2008) tiene más de 20 problemas de sincronización,
...
- /Tutorial de hilos de Perl/
  #+latex: \otrorec{http://perldoc.perl.org/perlthrtut.html}
  #+html: <br/><a href="http://perldoc.perl.org/perlthrtut.html">http://perldoc.perl.org/perlthrtut.html</a><br/>
  John Orwant (1998); The Perl Journal
- /Python higher level threading interface/
  #+latex: \otrorec{http://docs.python.org/2/library/threading.html}
  #+html: <br/><a href="http://docs.python.org/2/library/threading.html">http://docs.python.org/2/library/threading.html</a><br/>
  Python Software Foundation (1990-2014); Python 2.7.6 Documentation
- /Spin Locks & Other Forms of Mutual Exclusion/
  #+latex:  \otrorec{http://www.cs.fsu.edu/~baker/devices/notes/spinlock.html}
  #+html: <br/><a href="http://www.cs.fsu.edu/~baker/devices/notes/spinlock.html">http://www.cs.fsu.edu/~baker/devices/notes/spinlock.html</a><br/>
  Theodore P. Baker (2010); Florida State University
- /Dining philosophers revisited/
  #+latex: \otrorec{https://dl.acm.org/citation.cfm?id=101091}
  #+html: <br/><a href="https://dl.acm.org/citation.cfm?id=101091">https://dl.acm.org/citation.cfm?id=101091</a><br/>
