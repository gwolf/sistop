...
determinado sólo puede estar ejecutándose un número de procesos igual
# sintaxis
...
ilusión de simultaneidad en la ejecución /[[./03_planificacion_de_procesos.org][Planificación de procesos]]/.
# legibilidad
...
- Información de planificación (scheduling) :: La prioridad del
     ...
     profundizaremos en el tema en la unidad /[[./03_planificacion_de_procesos.org][Planificación de
     procesos]]/.
...
# tipo
     la unidad /[[./04_administracion_de_memoria.org][Administración de memoria]]/.
...
gracias a no tener que reemplazar al PCB activo, pero además de esto,
ganan mucho más por la ventaja de compartir espacio de memoria sin
tener que establecerlo explícitamente a través de /mecanismos de
comunicación entre procesos/ (/IPC/ — /Inter Process
Communications/). Dependiendo de la plataforma, a veces los hilos de
usuario inclusive utilizan multitarea cooperativa para pasar el
control dentro de un mismo proceso. Cualquier llamada al sistema
/bloqueante/ (como obtener datos de un archivo para utilizarlos
inmediatamente) interrumpirá la ejecución de todos los hilos, dado que
el control de ejecución es entregado al sistema operativo.

Otra alternativa es la creación de hilos /informando/ al sistema
# mejora de legibilidad: no quedaba claro que lo anterior era un paso
operativo, típicamente denominados /hilos de kernel/ (/kernel
threads/). Esto se hace a través de bibliotecas de sistema que los implementan de
# sintáxis
...
- Jefe / trabajador :: Un hilo tiene una tarea distinta de todos los
     ...
     los trabajadores a hacer el trabajo /de verdad/; como mucho,
     puede llevar contabilidad de los trabajos realizados.

     Típicamente, los hilos realizan su operación, posiblemente
...
- Equipo de trabajo :: Al iniciar la porción multihilos del proceso,
     ...
     utilizado para cálculos matemáticos (ej.: criptografía,
# uhm... me suena que asi queda mejor poner ej., pero no se cual es la forma correcta
...
# sintaxis
     se plantea en un principio, esto es, el paso de /división de
...
# legibilidad
     ...
     hilos no /terminan/, sino que /son esperados/ y continúa la
     ejecución lineal.
...
# #+caption: Esquematización de los modelos de hilos: Jefe/trabajador,
# Línea de ensamblado, Equipo de trabajo ([[http://cs.uns.edu.ar/~jechaiz][Imagen de Javier Echaiz]])
...
en la competencia por recursos, a veces un programa no requiere 
 esa ilusión  — Parte del desarrollo de un programa puede depender
de datos obtenidos en fuentes externas a éste, y la cooperación con
hilos o procesos externos es fundamental.

Para algunos de los problemas presentados a continuación, presentaremos soluciones 
...
- Operación atómica :: Operación que tenemos la garantía que se
     ejecutará como una sóla unidad de ejecución (o fallará completamente, sin resultados o estados parciales). Esto no
...
#quité ese párrafo, pues es como que queda afuera el algoritmo de Peterson
...
muy peligroso, dado que permitiría que el sistema operativo pueda perder 
...
ejecutando en un momento dado salga de ella. Si no hay ningún hilo o proceso en esa sección crítica (o cuando un hilo sale de esa sección crítica), uno sólo de los que esperan podrá ingresar.

Como vimos en el ejemplo anterior, para que una mutex sea efectivo
tiene que ser implementado a través de una /primitiva/ a un nivel
inferior, implicando al planificador.
# puse inferior, ya que mas abajo tambien se usa un "nivel inferior" para referirse al SO.
...
dado que nos evita el uso de un lock explícito —
# esto me sonaba muy raro ... no sé muy bien qué quería decir ...
# en este punto todavía no definimos variable de condición
...
# quité la oración, me parece que confunde más que lo que ayuda
...
que hemos mayormente obviado (como ser, no sólo la cantidad de procesos que
...
determinada sección crítica alternan entre su ejecución
...
multiprocesamiento real (por problemas de coherencia de caché). Hace uso de la espera activa, lo que consume recursos innecesariamente, pero en una
situación como la descrita en que no hay paso de señales, eso es
inevitable.
...
solución válida. Algunos de estos problemas fueron
...
bibliografía desarrollándolos, tanto tal como fueron planteados como con
...
*** Problema lectores-escritores
  ...
  hay lectores ni escritores accediendo al buffer (o cuarto) . Implementamos los mutexes con semáforos.
...
=agente()= (el sistema operativo) seguirá enviándo una señal por
...
exclusión mutua. A través de /variables de condición/ podemos también
...
pthread_cond_t  CV[NTHREADS];    /* Una variable de condición por filosofo */
...
# este párrafo anterior está incompleto?
...
- Instrucciones atómicas ::
     ...
     existe en algunos procesadores (ej. todos los sucesores del Intel i386) y es empleada hoy en día, pero su uso está limitado a las
...
aunque esto dificultaría la programación. Podría verse a la [[#one-shot][solicitud
de una vez]] como un caso extremo de asignación jerárquica con jerarquía
plana] La condición básica es que, una vez que un proceso obtiene un
recurso de determinado nivel, sólo puede solicitar recursos
...
el planificador puede marcar qué flujos -de recursos- entre dos (o más) procesos son
...
# decía scanner, que es mejor como ejemplo, pero en la gráfica dice plotter
[[./img/gnuplot/tray_proc_evasion_bloqueo.png]]

El análisis de la interacción entre dos procesos se representa como en
la figura anterior; el avance es marcado en sentido horizontal para el
...
En la figura presentada, si bien $P_1$ y $P_2$ están esperando que se
...
monitoreen /de una forma inteligente/ y tomen acciones (ya sea
alertar a los administradores o, como lo revisamos en la sección de
[[#det_y_rec_bloq][detección y recuperación de bloqueos]], abortar -y posiblemente
reiniciar- la ejecución de aquellos procesos que no puedan
recuperarse).
