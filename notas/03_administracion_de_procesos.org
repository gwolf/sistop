...
bastar con guardar la información relacionada con la /contabilidad/ de
...
En esta parte se desarrollan los conceptos relacionados con procesos,
hilos, concurrencia y sincronización — se abordarán en el capítulo
\ref{PLAN} (/Planificación de procesos/) las técnicas y algoritmos que
emplea el sistema operativo para determinar cómo y en qué orden hacer
los cambios de proceso que nos brindan la ilusión de simultaneidad.
...
- Información de planificación (scheduling) :: La prioridad del
     ...
     profundizará el tema en el capítulo \ref{PLAN} (/Planificación de
     procesos/).

- Información de administración de memoria :: La información de mapeo
     de memoria (páginas o segmentos, dependiendo del sistema
     operativo), incluyendo la pila (/stack/) de llamadas. Se abordar
     el tema en el capítulo \ref{MEM} (/Administración de memoria/).
...
nuestra aplicación, e incluso se pueden /anidarl/ (esto es, se podría
...
o más eventos que ocurren a la vez sino a dos o más eventos cuyo órden
es /no determinista/, esto es, eventos acerca de los cuales /no se
puede predecir el órden relativo en que ocurrirán/. Si bien dos
...
Para cada problema se mostrará /una forma/ de resolverlo los problemas
propuestos anteriormenete, pero para todos estos problemas hay más de
...
A modo de ilustración de la gravedad del
...
**** Pero yo tengo muchos núcleos

Otra cuestión que puede parecer
...
Antes de presentar posibles soluciones, se verán algunos conceptos
importantes.
...
perfectamente entendible) y en cambio se presentará la solución
...
- Espera activa :: La solución presentada mantiene todavía el problema
                   ...
                   cambie una bandera y en un sistema con prioridades
                   la cosa puede ser bastante dañina. Una forma de
...
Para mayores detalles acerca de las razones, ventajas y desventajas
                   ...
                   referirse a [[http://www.cs.fsu.edu/~baker/devices/notes/spinlock.html#%281%29][Spin Locks & Other Forms of
...
- Solución para equipos multiprocesadores :: Esta solución (y también la de
     ...
     un procesador.
** Más conceptos de concurrencia
*** /Mutexes/
Una de las alternativas que suele ofrecer un lenguaje concurrente o
sistema operativo para evitar la /espera ocupada/ a la que obliga el
algoritmo de Peterson (o similiares) se llama /mutex/ o también
/lock/ (candado).
...
ejecutando salga de ella. Si no hay ningún hilo o proceso en esa
sección crítica (o cuando un hilo sale de esa sección crítica), uno
...
exclusivamente relacionados con esta parte: son conceptos que
...
*** Planteo
     ...
     - Los escritores deben tener acceso exclusivo a la sección
       crítica.
     - Como refinamiento al planteamiento: se debe evitar que un
...
Se recomienda el libro «[[Little_Book_of_Semaphores_-_Allen_Downey.pdf][The little book of semaphores]]» de Allen Downey
(2008) para profundizar en este tema. En este libro, se encuentran
muchos ejemplos que ilustran el uso de semáforos no sólo para resolver
problemas de sincronización, sino como un mecanismo simple de
comunicación entre procesos. También se desarrollan distintas
soluciones para los problemas clásicos (y no tan clásicos).
...
se declara un método de la siguiente manera (Silberschatz):
...
se llame a =sc.safeMethod()=, la máquina virtual verificará si ningún
...
tl sistema mantendrá /en espera/ a $B$ si $t_B \le 2$ mientras $2 \le
...
sin peligro en el órden presentado.
...
/órden y tiempo/ en que cada uno de los procesos requeriría los
recursos.
