...
que hemos mayormente obviado (como son no sólo la cantidad de procesos
que emplearán determinada sección crítica, sino una /numeración/ que
...
entrar a determinada sección crítica, siempre que estos alternen entre
el tiempo en que ejecutan código que puede proceder en paralelo, basta
contar con las dos siguientes variables compartidas entre ellos:
...
do {
    ...
    /* Ejecutando en seccion critica */
    listo[i] = FALSE;
    /* Ejecutando en seccion no critica */
} while TRUE;
#+end_src
...
- Instrucciones atómicas ::
     ...
     #+begin_src asm
       enter_region:      ; Etiqueta de entrada a la verificacion
         tsl reg, flag    ; Test and Set Lock; 'flag' es la
                          ; variable compartida, es cargada al
                          ; registro 'reg' y, atomicamente,
                          ; convertida en 1.
         cmp reg, #0      ; Era la bandera igual a 0 al entrar?
         jnz enter_region ; En caso de que no fuera 0 al ejecutar
                          ; el tsl, vuelve a ejecutar desde
                          ; enter_region
         ret              ; Termina la rutina. Esto significa que
                          ; 'flag' era cero al entrar. Si llegamos
                          ; hasta aqui, tsl fue exitoso y flag
                          ; queda marcado como no-cero. Tenemos
                          ; acceso exclusivo al recurso protegido
                          ; por flag.
...
T_A \le 6$, y mantendrá a $A$ en espera si $t_A \le 2$ mientras $2 \le
t_B \le 6$.
