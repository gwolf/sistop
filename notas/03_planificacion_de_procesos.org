...
2 /ticks/, y la duración de cada /quantum/ es de 5 ticks, en un
...
#+begin_src ditaa :file ltxpng/srr.png :cmdline -E
  ...
  +----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+
#+end_src
#+html: <p align="center">Ronda egoísta (SRR) con <em>a = 2</em> y <em>b = 1</em></p></div>
...
#+begin_src ditaa :file ltxpng/fb_bas.png :cmdline -E
  ...
  +----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+
#+end_src
#+html: <p align="center">Retroalimentación multinivel (FB) básica</p></div>
...
#+begin_src ditaa :file ltxpng/fb_exp.png :cmdline -E
  ...
  +----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+
#+end_src
#+html: <p align="center">Retroalimentación multinivel (FB) con <em>q</em> exponencial</p></div>
...
son gestionados por el sistema operativo como si fueran procesos.
...
Cuando hablamos de los /hilos de núcleo/, nos encontramos con que el
despachador en realidad se encarga de planificar /hilos/, esto es, un
proceso que no crea hilos internamente es tratado como si fuera un
solo hilo.
...
Una respuesta es que todos los hilos deben ser atendidos sin exceder
el tiempo que sería asignado a un sólo proceso. Esto se conoce como
/Ámbito de contención por proceso/ (/Process Contention Scope/,
/PCS/). Este modelo es muchas veces denominado /uno a muchos/, dado
que dentro del espacio de ejecución de un proceso son /agrupados/
muchos hilos.

En contraposición, si cada hilo es visto por el planificador como un
proceso independiente, hablamos de un /Ámbito de contención por
sistema/ (/System Contention Scope/, /SCS/). Este modelo es también
llamado /uno a uno/, dado que los hilos son tratados, para propósitos
de planificación, cual procesos normales.

Los sistemas operativos Windows, Linux y Solaris implementan
únicamente contención por sistema (SCS); algunos Unixes históricos
(IRIX, HP-UX y Tru64) emplean contención por proceso (PCS).
...
incluye en su texto las siguientes figuras comparando algunos
aspectos importantes de los diferentes despachadores:
...
#+BEGIN_SRC ditaa :file ltxpng/pipeline.png :cmdline -E
  Instr
     |  +--------+--------+--------+--------+--------+
  1  |  | IF     | ID     | EX     | MEM    | WB cGRE|
     |  +--------+--------+--------+--------+--------+--------+
  2  |           | IF     | ID     | EX     | MEMcGRE| WB     |
     |           +--------+--------+--------+--------+--------+--------+
  3  |                    | IF     | ID     | EX cGRE| MEM    | WB     |
     |                    +--------+--------+--------+--------+--------+--------+
  4  |                             | IF     | ID cGRE| EX     | MEM    | WB     |
     |                             +--------+--------+--------+--------+--------+--------+
  5  |                                      | IF cGRE| ID     | EX     | MEM    | WB     |
     |                                      +--------+--------+--------+--------+--------+
     V
          1       2        3         4        5        6        7        8        9
   Reloj --------------------------------------------------------------------------------->
...
#+BEGIN_SRC ditaa :file ltxpng/hyperthread.png :cmdline -E
hilo 1      +----+----+----+----+----+----+----+----+----+----+
----------->|cGRE|cRED|cGRE|cRED|cGRE|cRED|cGRE|cRED|cGRE|cRED|
            +----+----+----+----+----+----+----+----+----+----+
...
+----+                   +----+
|cGRE| Ciclo de cómputo  |cRED| Espera por memoria
+----+                   +----+
#+END_SRC

La planificación de los hilos hardware sale del ámbito del presente
curso; implica cuestiones de complejidad tal como el ordenamiento
...
estado del procesador correspondiente a otro de los hilos.
