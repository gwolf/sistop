...
#+include: fig/mem_base_a_limite.ditaa
...
#+include: fig/proceso_en_memoria.ditaa
...
#+include: fig/tipo_resol_direcc.dot
...
#+include: fig/compactacion_memoria.ditaa
...
#+attr_latex: width=0.7\textwidth
#+include: fig/segmentacion_de_memoria.dot

#+results:
file:ltxpng/segmentacion_de_memoria.png
#+html: <p align="center">Ejemplo de segmentación</p></div>
...
#+include: fig/direccion_a_pag_y_despl.ditaa
...
#+include: fig/hardware_de_paginacion.dot
#+html: <p align="center">Esquema del proceso de paginación, ilustrando el rol del MMU</p></div>
...
#+include: fig/ejemplo_de_paginacion.ditaa
#+html: <p align="center">Ejemplo (minúsculo) de paginación, con un espacio de direccionamiento de 32 bytes y páginas de 4 bytes (Silberschatz, p.292)</p></div>
...
#+include: ./fig/paginacion_jerarquica.dot
#+html: <p align="center">Paginación en dos niveles: Una tabla externa de 10 bits, tablas intermedias de 10 bits, y marcos de 12 bits (esquema común para procesadores de 32 bits)</p></div>
...
#+include: fig/memoria_compartida.ditaa
...
#+include: fig/cow_recien_hecho.ditaa
...
#+include: fig/cow_pagina_modificada.ditaa
...
#+attr_latex: width=0.7\textwidth
#+include: fig/esquema_gral_mem_virtual.dot
#+html: <p align="center">Esquema general de la memoria, incorporando espacio en almacenamiento secundario, representando la memoria virtual</p></div>
...
resultante. Veremos que, empleando memoria virtual, de cierto modo la
memoria física se vuelve sólo una /proyección parcial/ de la memoria
lógica, potencialmente mucho mayor a ésta.
...
#+attr_latex: width=0.6\textwidth
#+include: fig/respuesta_a_fallo_de_pagina.dot
#+html: <p align="center">Pasos que atraviesa la respuesta a un fallo de página</p></div>
...
#+include: fig/anomalia_belady.gnuplot
...
#+include: fig/reemplazo_pag_fifo.ditaa
#+html: <p align="center">Algoritmo FIFO de reemplazo de páginas.</p></div>
...
#+include: fig/reemplazo_pag_opt.ditaa
#+html: <p align="center">Algoritmo óptimo de reemplazo de páginas (OPT)</p></div>
...
#+include: fig/reemplazo_pag_lru.ditaa
#+html: <p align="center">Algoritmo de reemplazo de páginas menos recientemente utilizadas (LRU)</p></div>
...
[[#conjunto_activo][conjunto activo]]
...
#+attr_html: height="382"
#+attr_latex: width=0.7\textwidth
#+include: fig/hiperpaginacion.gnuplot
...
#+attr_html: height="354"
#+attr_latex: width=0.7\textwidth
[[./img/conjunto_activo.png]]
...
recalcado a lo largo del curso es la de implementar protección entre
...
A lo largo de la presente unidad hemos revisado que el MMU incluye ya
...
Recordando lo mencionado en la sección en que presentamos al [[#espacio_en_memoria][espacio
en memoria de un proceso]], profundicemos un poco en cómo está
estructurada la /pila de llamadas/ (/stack/).
...
#+include: fig/stackframe.ditaa
#+html: <p align="center">Marco del stack con llamada a =func(1,2,3)= en x86-32</p></div>
...
#+include: fig/overflow_500.ditaa
#+html: <p align="center">Estado de la memoria después del <tt>strcpy()</tt></p></div>
...
Hay dos técnicas principales: /Brincar/ a un punto determinado del
programa, y /brincar/ hacia dentro del stack. Veamos la primera:
...
de este =if= a ensamblador es, primero, brincar hacia la etiqueta
=valid_user=, y (empleando al valor que ésta regrese en =%EBX=) ir a
la siguiente instrucción, o brincar a la etiqueta =FAIL=. Esto puede
hacerse con la instrucción =BNE $0, %EBX, FAIL= (/Branch if Not
Equal/, /Brinca si no es igual/, que recibe como argumentos dos
...
proporicione. Para esto, el ataque más frecuente es brincar /hacia
adentro del stack/.

#+html: <div class="figure">
#+attr_latex: width=0.7\textwidth
#+caption: Ejecutando el código arbitrario inyectado al buffer
#+include: fig/overflow_jump.ditaa
#+html: <p align="center">Ejecutando el código arbitrario inyectado al buffer</p></div>
...
Empleando este mecanismo, el MMU puede evitar la ejecución de código
en el área de stack, lo cual anula la posibilidad de /brincar al
stack/. Esta protección desafortunadamente no es muy efectiva: Una vez
que tiene acceso a un buffer vulnerable, el atacante puede /brincar a
...
una jaula en la mina. Como el canario es muy sensible ante la falta de
...
#+include: fig/overflow_canary.png
#+html: <p align="center">Marco de stack con un <em>canario</em> aleatorio protector de 12 bytes (<tt>qR'z2a&amp;5f50s</tt>): Si este es sobreescrito por un buffer desbordado, se detendrá la ejecución del programa.</p></div>
...
- [[http://kingcope.wordpress.com/2013/01/24/attacking-the-windows-78-address-space-randomization/][Attacking the Windows 7/8 Address Space Randomization]] explica cómo
  ...
  cual brincar. Puede interesarles también leer la [[http://tech.slashdot.org/story/13/01/25/2146222/hacker-bypasses-windows-78-address-space-layout-randomization][discusión en
