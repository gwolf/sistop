...
MMU por regiones, evitando la modificación de código ejecutable — El
MMU también puede evitar la ejecución de código en el área de stack,
lo cual anula la posibilidad de /brincar al stack/. Esta protección
existe desde hace muchos años, y desafortunadamente no es muy
efectiva: Una vez que tiene acceso a un buffer vulnerable, el atacante
puede /brincar a libc/, esto es, por ejemplo, proporcionar como
parámetro el nombre de un programa a ejecutar, e indicar como retorno
la dirección de la función =system= o =execve= de la =libc=.

Las secciones de texto no ejecutables son, pues, un obstáculo ante un
