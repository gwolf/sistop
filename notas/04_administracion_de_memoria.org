...
mucho menor: Si el segmento de texto es de sólo lectura, una vez que
éste fue copiado una vez al disco, ya no hace falta volver a hacerlo:
Tenemos la certeza de que no será modificado por el proceso en
ejecución, por lo que basta marcarlo como /no presente/ en las tablas
de segmentos en memoria para que cualquier acceso ocasione que el
...
correspondientemente muy caros. El manejo de páginas más pequeñas (que
es lo normal), y muy especialmente el uso de espacios de
direccionamiento de 64 bits, harían prohibitivo este enfoque. Además,
nuevamente, cada proceso tiene una tabla de páginas distinta — Se
haría necesario hacer una transferencia de información muy grande en
...
encontrarse la página en el TLB, el MMU lanza una /falla de página/
...
TLB está lleno y se produce una falla de página. Uno de los esquemas
...
de la página ocupa 48 bits[fn:: 20 bits identificando a la página y
otros 20 bits identificando al marco, mas cuatro bytes para /alinear/
las entradas al byte más cercano, dado que la memoria se organiza por
bytes] (esto es, 6 bytes), cada proceso requeriría de 6MB (6 bytes por
cada una de las páginas) sólamente para representar su mapeo de
memoria. Esto, especialmente en procesos pequeños, resultaría más
gravoso para el sistema que los beneficios obtenidos de la
paginación.
...
U\rightarrowM$, una función que /mapea/ o /proyecta/ al conjunto $U$
...
#+begin_src ditaa :file ltxpng/memoria_compartida.png :cmdline -E
  ...
  +--------+-------+   +--------+-------+   +--------+-------+     |   1 cYEL   |
  |cYEL  1 | 3 cPNK|   |cGRE  1 | 3 cPNK|   |cRED  1 | 3 cPNK|     +------------+
  +--------+-------+   +--------+-------+   +--------+-------+     |   2 cPNK   |
  |cYEL  2 | 5 cPNK|   |cGRE  2 | 5 cPNK|   |cRED  2 | 5 cPNK|     +------------+
  +--------+-------+   +--------+-------+   +--------+-------+     |   3 cPNK   |
  |cYEL  3 | 2 cPNK|   |cGRE  3 | 2 cPNK|   |cRED  3 | 2 cPNK|     +------------+
  +--------+-------+   +--------+-------+   +--------+-------+     |   4 cGRE   |
