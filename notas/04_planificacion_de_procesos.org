...
corto seguido de un periodo sin avance). Claro está, el procesador
...
despachar primero a los procesos más cortos, mantiene la lista de
...
poco común encontrar a ninguno de estos mecanismos en un /estado puro/
...
\caption {Tres modelos de mapeo de hilos a procesos. (Imágenes: Beth Plale; ver \emph{Otros recursos})}
...
- Muchos a muchos :: Este mecanismo permite que existan hilos de ambos
     ...
     Este modelo se presenta en la figura
     \ref{PLAN_hilos_muchos_a_muchos}.

No se detalla en el presente texto respecto a los primeros — cada
marco de desarrollo o máquina virtual (ver sección
...
1. En un sistema interactivo, los procesos típicamente están en
   ...
   embargo, en el transcurso del capítulo estos fueron casi siempre
   tratados como /procesos cortos/. ¿Qué significa esto? ¿Cuál sería
   un ejemplo de /proceso largo/?
...
3. ¿Cuáles de los algoritmos estudiados son más susceptibles a la
   ...
   Identifique por lo a menos dos en cada caso.
...
- Núcleo prevenible, tiempo real, y optimización fina ::
  ...
  Linux. Los últimos diez años han sido de fuerte profesionalización y
  optimización.
  ...
  diciembre del 2002, [[http://www.aeolean.com/html/RealTimeLinux/RealTimeLinuxReport-2.0.0.pdf][Introduction to Linux for Real-Time Control:
  Introductory Guidelines and Reference for Control Engineers and
  Managers]]. Detalla muy bien cuáles son los requisitos (y las razones
  detrás de ellos) para la implementación de sistemas operativos de
  tiempo real, e incluye una revisión del panorama de este campo, muy
  interesante y muy buen recurso para construir desde ahí.

  Otra referencia muy recomendable al respecto es el artículo
  de Jonathan Corbet, publicado en agosto del 2013 por /Linux Weekly News/,
  [[https://lwn.net/Articles/563185/][Optimizing preemption]].
...
- Las clases de planificación en Linux y =SCHED_DEADLINE= ::
  ...
  *edf* (/Primer Plazo Primero/, /Earliest Deadline First/).
  ...
  - [[https://lwn.net/Articles/575497/][Artículo en *lwn* de diciembre de 2013 anticipando la inclusión de
    SCHED_DEADLINE]], que presenta los conceptos, e incluye una
    discusión al respecto que puede resultarles interesante y útil
  - [[https://en.wikipedia.org/wiki/SCHED_DEADLINE][La página de la clase de planificación SCHED_DEADLINE en
    Wikipedia]]
  - [[https://git.kernel.org/cgit/linux/kernel/git/torvalds/linux.git/plain/Documentation/scheduler/sched-deadline.txt?id=712e5e34aef449ab680b35c0d9016f59b0a4494c][La documentación de SCHED_DEADLINE en el núcleo de Linux]]
