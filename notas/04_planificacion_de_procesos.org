...
en la figura \ref{PLAN_estados_proceso}), podríamos ubicar a estos tres
...
hoy en día. Veamos brevemente, antes de abordar al esquema de
retroalimentación multinivel, cómo opera un sistema con múltiples
colas de prioridad.
...
La figura \ref{PLAN_planif_multicolas} nos ilustra cómo se presentaría
...
esquema nos da los siguientes resultados:
...
Dado que ahora tenemos que representar la cola en la que está cada uno
de los procesos, en la figura \ref{PLAN_planif_fb_bas} presentamos
sobre cada una de las líneas de proceso la prioridad de la cola en que
se encuentra antes del /quantum/ en que se ejecuta:
...
comparación de este mecanismo con los que abordamos previamente.
...
Recordemos que, si bien presentamos una comparación de los diferentes
algoritmos ante una determinada carga de procesos, no podemos asumir
...
un esquema SRR (y mucho más favorecido por un SPN o PSPN) que uno en
el cual predominen los procesos largos.
...
obteniendo como resultado las figuras que aquí reproducimos
...
/Hyperthreading/, que abordaremos brevemente en la sección
...
Supongamos que un dispositivo genera periódicamente determinada
...
por medio del cual podrá alertar al usuario.
...
/aperiódica/, por ocurrencia única (/necesito que este proceso, que me
...
- [[http://stimulationofcp.sourceforge.net/][Simulación de algoritmos de planificación]]: Programa en Java escrito
  por P. A. Krishnan para un curso de Sistemas Operativos, presentando la
