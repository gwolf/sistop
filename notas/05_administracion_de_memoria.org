...
Con la llegada de la era de las /computadoras personales/, Intel y
otros fabricantes introdujeron a mediados de los años 1980 los procesadores
con registros de 32 bits. Por ejemplo, la arquitectura IA-32 tiene su 
inicio oficial con el procesador 80386 ( o simplemente 396). 
...
producían memorias de capacidad mucho menor.
...
- Espacio de /libres/ ::  Espacio de memoria que se emplea
     ...
     proceso (/Heap/). Este espacio se ubica por encima de la sección de datos,
...
se pasa a lenguaje de máquina, no existe diferencia real entre una variable
o una rutina. La diferencia se establece por el uso que se dé a la
...
de memoria disponible, rodeados de regiones de memoria usada por los
...
en su espacio de memoria. Una estrategia ante esta situación podría ser 
...
en estos lenguajes, se realizan referencias a los /nombres/ 
de las variables o las funciones. No obstante, cuando se compila el
programa para una arquitectura específica de procesador, éste puede 
implementar mecanismos para evitar la escritura accidental de las 
...
la pila /stack/ y el espacio de asignación dinámica /heap/).
...
otra sección contiene el código en lenguaje de máquina, otra sección 
...
configurar otras secciones como la pila o el montículo. 
...
La segmentación es un concepto que se aplica directamente a la arquitectura
del procesador. Permite separar las regiones de la memoria /lineal/ en
 /segmentos/, cada uno de los cuales puede tener diferentes tipos
(lectura, escritura, ejecución), y diferentes niveles (modo usuario, modo
administrador) de acceso. La segmentación también ayuda a incrementar la 
/modularidad/ de un programa: Es muy común que las bibliotecas 
/ligadas dinámicamente/ estén representadas en segmentos independientes.
...
tipo de referencias se denominan /direcciones lógicas/ y consisten en un
...
alguna estructura de datos - Como la Tabla Global de Descriptores GDT o la 
Tabla Local de Descriotores LDT en arquitecturas compatibles Intel), 
...
Cuando se activa la paginación, la MMU transforma las direcciones lógicas
a direcciones lineales (por medio de tablas de segmentos), y luego las
direcciones lineales se transforman a direcciones físicas usando una
tabla de páginas.
...
marcos, convirtiendo la /dirección lineal/  en la /dirección física/ 
(la ubicación en que /realmente/ se encuentra en la memoria del sistema).
...
dirección lineal, y el desplazamiento con los 2 bits /menos
significativos/).
...
de las páginas seguirá siendo compartido. Esto se puede logra marcando
/todas/ las páginas compartidas como "sólo lectura", con lo cual cuando
uno de los dos procesos intente modificar la información de alguna página 
se generará un fallo. El sistema operativo copiará entonces sólo la 
página en la cual se encuentra la dirección de memoria que causó el fallo,
y esta vez marcará la página como "lectura y escrtura".
