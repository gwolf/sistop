...
disco[fn:: En la unidad /Sistemas de entrada y salida/ se estudiará cómo se
interactúa con estas]. Cabe mencionar que cuando se encuentre en un texto
...
del sistema antes de ser utilizados. En esta unidad se mostrará cómo
...
hasta $2^{32}}$ bytes de RAM. No obstante, debido a las 
limitaciones tecnológicas (y tal vez estratégicas) para producir 
memorias con esta capacidad, se tomó más de veinte años que las 
...
base/ contendría 504214, y el /registro límite/ 65535[fn:: De hecho esto sucedía
en los procesadores x86 con registros de 16 bits, que pueden almacenar un
valor máximo de 65535 ($2^{16} - 1$. Si hubiera una
...
- Pila de llamadas :: Consiste en un espacio de memoria que se usa para
     almacenar la secuenciade funciones que han sido llamadas dentro 
...
se encuentran los sistemas que utilizan /intercambio/ (/swapping/) entre la
...
Los bits de cada dirección se separa en un identificador de página y un
...
presentado en el libro de Silberschatz, Galvin y Gagné. Este presenta
...
Se puede tomar la paginación como una suerte de resolución de
...
Sin embargo, la sobrecarga administrativa en que se incurre por
...
disco duro), a través de un esquema de /intercambio/ (/swapping/).
...
Se debe considerar que existe una gran cantidad de /código durmiente/: Código
...
sobre demanda, ya que es la pieza que posibilita una verdadera separación
...
está apagada, se garantiza la información en disco es idéntica su copia en 
...
marcos disponibles se tendrá una menor cantidad de fallos de página,
con algunas de cadenas de referencia y bajo ciertos algoritmos puede
haber una /regresión/, en la cual la cantidad de fallos aumenta aún
con una mayor cantidad de marcos.
...
Es importante recalcar que si bien la anomalía de Belady se nos
...
A lo largo de la presente unidad se ha mencionado que la MMU incluye ya
...
simple como sea posible, manteniéndose , para que pudiera ser empleado 
