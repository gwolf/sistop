...
/Unidad de Manejo de Memoria/ (MMU). Se describirá cómo se manejaban
...
un /registro base/ y un /registro límite/: si la arquitectura ofrece dos
...
ejecutando en /modo supervisor/), la MMU puede comparar sin penalidad /cada 
...
- Espacio de /libres/ ::  Espacio de memoria que se emplea
     ...
     dinámico manual de la memoria/ (como C), esta área es la que se maneja a
...
/resueltas/ o traducidas.  Existen diferentes estrategias de
resolución, que se pueden clasificar a grandes rasgos[fn:: Esta
explicación simplifica muchos detalles; para el lector interesado en
profundizar en este tema, se recomienda el libro /Linkers and Loaders/
(Ligadores y cargadores) de John R. Levine (1999). El libro está
disponible en línea desde el sitio Web del autor,
http://www.iecc.com/linker/ ] en:
...
primero a la /más severa/: si como resultado de esa solicitud iniciará
...
p.292). Éste presenta un esquema minúsculo de paginación: un /espacio
de direccionamiento/ de 32
...
4MB), existirían 1024 páginas posibles[fn:: 4MB es $2^{22}$ bytes;
...
Se estaría penalizando a /cada acceso a memoria/ con un acceso de
...
Como sea, dado que el TLB es limitado en tamaño, es necesario explicitar una
política que indique dónde guardar las nuevas entradas (esto es, qué 
entrada reemplazar) una vez que el
...
\begin{figure}
\centering
\subfigure[Inmediatamente después de la creación del proceso hijo por \texttt{fork()} \label{MEM_cow_recien_hecho}]{\includegraphics[width=\textwidth]{img/ditaa/cow_recien_hecho.png}}
\subfigure[Cuando el proceso hijo modifica información en la primer página de su memoria, se crea como una página nueva. \label{MEM_cow_pagina_modificada}]{\includegraphics[width=\textwidth]{img/ditaa/cow_pagina_modificada.png}}
...
\begin{figure}
\centering
\subfigure[Relación ideal entre el número de marcos y fallos de página. \label{MEM_expectativa_fallas_marcos}]{\includegraphics[width=0.6\textwidth]{img/gnuplot/expectativa_fallas_contra_marcos.png}}
\subfigure[Comportamiento del algoritmo FIFO exhibiendo la anomalía de Belady al pasar de 3 a 4 marcos. \label{MEM_anomalia_belady}]{\includegraphics[width=0.6\textwidth]{img/gnuplot/anomalia_belady.png}}
  ...
  <tr>
    ...
    <td style="padding: 1em">
      <span id="MEM_anomalia_belady">
        ...
        <p>Comportamiento del algoritmo FIFO exhibiendo la anomalía de Belady al pasar de 3 a 4 marcos.</p>
      </span>
    </td>
  </tr>
...
5. Suponga que un sistema paginado con un rango de direcciones de 4GB
...
- Mecanismos para mantener la coherencia en caché ::
  ...
  fueron diseñados: MSI, MESI (Illinois), MOSI (Berkeley), MOESI y
  MESIF.
...
- Otras categorías de vulnerabilidad ::
  ...
  - En abril de 2014 se dio a conocer un fallo en la biblioteca
    criptográfica /OpenSSL/, particularmente en la rutina que
    implementa el /Heartbeat/. Dada la gran base instalada de usuarios
    de OpenSSL, y lo sensible de la información que expone al
    atacante, desde su descubrimiento se dio por hecho que esta es una
    de las vulnerabilidades con mayor impacto a nivel mundial. Dado el
