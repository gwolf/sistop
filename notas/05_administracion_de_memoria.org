...
$65~536$ bytes (64KB). Estos procesadores fueron comunes en las
...
con arquitectura de 32 bits. Por ejemplo, la *ia-32* de Intel tiene su
...
64K ($65~535$ bytes) a partir de la dirección $504~214$ (492K), el
/registro base/ contendría 504214, y el /registro límite/ $65~535$. Si
hubiera una instrucción por parte de dicho proceso que solicitara una
dirección menor a $504~214$ o mayor a $569~749$ (556K), la *mmu*
lanzaría una excepción o /trampa/ interrumpiendo el procesamiento, e
indicando al sistema operativo que ocurrió una /violación de segmento/
(/segmentation fault/).[fn:: ¿Por qué /de segmento/? Véase la sección
\ref{MEM_segmentacion}.] El sistema operativo entonces procedería a
terminar la ejecución del proceso, reclamando todos los recursos que
tuviera asignados y notificando a su usuario.
...
memoria la siguiente instrucción a ejecutar. Por otro, estas
...
y cargadores/) de John R. Levine (1999). Está disponible en línea
desde el sitio Web del autor, http://www.iecc.com/linker/ ] en:
...
/inicio/ + 5986 bytes, y el proceso de carga incluiría sustituirla
...
Comienza a aparecer cuando más procesos terminan su ejecución, y el
sistema operativo libera la memoria asignada a cada uno de ellos. A
medida que los procesos finalizan, aparecen regiones de memoria
...
bloques asignados, se perderán del orden de /0.5N/ bloques por
...
bytes ($2^9$) y 16MB ($2^{24}$); al ser una potencia de dos, la *mmu*
...
p:292). Éste muestra un esquema minúsculo de paginación: un /espacio
de direccionamiento/ de 32 bytes (cinco bits), organizado en ocho
páginas de cuatro bytes cada una (esto es, la página es representada
con los tres bits /más significativos/ de la dirección, y el
desplazamiento con los dos bits /menos significativos/).
...
tiene sus datos distribuidos en orden desde la dirección 00000 (0)
hasta la 01111 (15), aunque en realidad en el sistema éstas se
...
únicamente ocho páginas posibles ($2^3$), por lo que resultaba sensato
...
—si para resolver una dirección lógica a su correspondiente dirección
física hace falta consultar la tabla de páginas en memoria, el tiempo
efectivo de acceso a memoria se duplica.
...
las propiedades de /localidad de referencia/ que se presentaron anteriormente,
...
(esto es, cinco bytes), cada proceso requeriría de 5MB (cinco bytes
...
Un programa que está programado y compilado de forma que permita que
...
copia a un nuevo marco, y deja de ser una página compartida, como se
puede ver en la figura \ref{MEM_cow_pagina_modificada}. El resto
...
/direccionar/ más memoria de la físicamente disponible, no se ha presentado
...
disco duro), a través de un esquema de /intercambio/ (/swap/) guardando
...
Se debe considerar que existe una gran cantidad de /código durmiente/
o /inalcanzable/: código
...
necesaria/ para comenzar la ejecución (posiblemente una página o ninguna),
 y que a lo largo de la ejecución, el paginador /es flojo/:[fn:: En
...
llevar a cabo un /reemplazo de páginas/ que permita continuar
...
que pueda liberar (una /página víctima/) y grabarla al espacio de 
...
Esto implica a una /doble/ transferencia al disco (una para grabar
...
se usará una /cadena de referencia/. Esto es, sobre una lista
...
\subfigure[Comportamiento del algoritmo *fifo* exhibiendo la anomalía de Belady al pasar de 3 a 4 marcos. \label{MEM_anomalia_belady}]{\includegraphics[width=0.7\textwidth]{img/gnuplot/anomalia_belady.png}}
  ...
  <tr>
    ...
    <td style="padding: 1em">
      <span id="MEM_anomalia_belady">
        ...
        <p>Comportamiento del algoritmo <b>fifo</b> exhibiendo la anomalía de Belady al pasar de 3 a 4 marcos.</p>
      </span>
    </td>
  </tr>
...
momento fue cargada, y cuando llegue el momento de reemplazar una
página vieja, se elige la que haya sido cargada hace más tiempo.
...
- Columna de referencia :: Una mejoría casi trivial sobre la anterior
     ...
     Por ejemplo, con una implementación de 4 bits, un
...
memoria física[fn:: Ya que de los 1024KB, o 256 páginas, que
...
- Reemplazo local :: Mantener tan estable como sea posible el
...
presenta hiperpaginación [fn:: Una traducción literal del
...
#+caption: Al aumentar demasiado el grado de multiprogramación, el uso del *cpu* cae abruptamente, entrando en hiperpaginación  (Silberschatz, p.349).
...
#+caption: Los picos y valles en la cantidad de fallos de página de un proceso definen a su /conjunto activo/.
#+attr_html: height="400"
#+attr_latex: width=0.7\textwidth
...
- [[http://www.phrack.org/issues.html?issue=49&id=14][Smashing The Stack For Fun And Profit]] (Aleph One, revista
  Phrack, 1996)
...
Ahora, lo expuesto anteriormente implica que el código /está demostrado
vulnerable/, pero se ha /explotado/ aún. El siguiente paso es,
...
a la /reutilización de código/ para aprovechar la implementación de
...
(pág. \pageref{MEM_tipo_resol_direcc}, ilustra el momento en que
ocurre cada uno de estos ligados:
el ligado estático es realizado por el /editor de ligado/, uniendo en
un sólo /módulo cargable/ al programa compilado (/módulo objeto/) con
...
El primer problema es conocido, en ambientes Windows, como el /infierno de las DLL/. Este
...
dependencias/ que verifican si existe en el sistema una versión
...
el código afectado estaría incluído como parte de /cada uno de los
...
1. Diagrame el acomodo del espacio en memoria de un proceso. ¿Qué
   ...
   datos/ con el /espacio de libres/ (/heap/)? ¿Y el /espacio de
...
5. Suponga un sistema paginado con un rango de direcciones de 4GB
   (4294967296 direcciones).
   - ¿Cuántas páginas tendrá el sistema si se utilizan páginas de 4096 bytes?
...
6. Explique la relación que hay entre direcciones virtuales y
...
7. Un equipo presenta rendimiento muy deficiente. Ante un análisis de
   ...
   uso de *cpu* está al 20% y el uso de la interfaz al disco duro que
   aloja a la memoria virtual al 95%. ¿En qué condición está el sistema?
   ...
   1. Reemplazar al *cpu* por uno 20% más rápido (pasar de 1GHz a 1.2GHz)
   2. Destinar un área del disco 25% más grande para la memoria
      virtual (pasar de 4 a 5GB)
   ...
   5. Instalar un 25% más de memoria principal (pasar de 2 a 2.5GB)
   6. Instalar un disco duro un 33% más rápido (cambiar un disco de
      5400 RPM por uno de 7200 RPM)
...
- Esquemas de asignación de memoria en una realidad *numa* ::
  ...
  interesante, de Cristoph Lameter: [[https://dl.acm.org/citation.cfm?doid=2500468.2500477][An overview of non-uniform memory
...
- Cargado y ligado de programas ::
  ...
  independiente de su posición/, etc.
...
- Mecanismos para mantener la coherencia en caché ::
  ...
  son tambíén conocidos por sus siglas, o por las universidades donde
...
- Relación con sistemas operativos en uso ::
  ...
  los parámetros de /sysctl/ relativos a la memoria virtual. Estos
...
- Otras categorías de vulnerabilidad ::
  ...
  Este capítulo presentó al /desbordamiento de pila/, una de las
  ...
  - En agosto del 2013, se descubrió lo que se popularizó como una
    ...
    línea de productos Apple se /caiga/. El siguiente artículo,
    publicado por Chris Williams, relata el proceso que llevó a
  ...
  - En abril de 2014 se dio a conocer un fallo en la biblioteca
    ...
    descubrimiento se dio por hecho que esta es una de las
    vulnerabilidades con mayor impacto a nivel mundial. Dado el
    ...
    esta problemática; otro artículo que la explica muy claramente es:

    http://www.theregister.co.uk/2014/04/09/heartbleed_explained/

** Lecturas relacionadas
- /The Grumpy Editor goes 64-bit/
  #+latex: \otrorec{https://lwn.net/Articles/79036/}
  #+html: <br/><a href="https://lwn.net/Articles/79036/">https://lwn.net/Articles/79036/</a><br/>
...
- /Using Valgrind to debug Xen Toolstacks/
  #+latex: \otrorec{http://www.hellion.org.uk/blog/posts/using-valgrind-on-xen-toolstacks/}
  #+html: <br/><a href="http://www.hellion.org.uk/blog/posts/using-valgrind-on-xen-toolstacks/">http://www.hellion.org.uk/blog/posts/using-valgrind-on-xen-toolstacks/</a><br/>
  Ian J. Campbell (2013). Presenta un ejemplo de uso de la herramienta
  /Valgrind/, para encontrar problemas en la asignación, uso y
...
- /Process memory usage/
  #+latex: \otrorec{http://troysunix.blogspot.mx/2011/07/process-memory-usage.html}
  #+html: <br/><a href="http://troysunix.blogspot.mx/2011/07/process-memory-usage.html">http://troysunix.blogspot.mx/2011/07/process-memory-usage.html</a><br/>
  ejemplos de pmap en diferentes Unixes

- /Página de manual de pmap en NetBSD/
  #+latex: \otrorec{http://www.daemon-systems.org/man/pmap.1.html}
  #+html: <br/><a href="http://www.daemon-systems.org/man/pmap.1.html">http://www.daemon-systems.org/man/pmap.1.html</a><br/>
  Más allá de
...
- /Linkers and Loaders/
  #+latex: \otrorec{http://www.iecc.com/linker/}
  #+html: <br/><a href="http://www.iecc.com/linker/">http://www.iecc.com/linker/</a><br/>
  John R. Levine
...
- /An anomaly in space-time characteristics of certain programs
  ...
  #+latex: \otrorec{http://dl.acm.org/citation.cfm?doid=363011.363155}
  #+html: <br/><a href="http://dl.acm.org/citation.cfm?doid=363011.363155">http://dl.acm.org/citation.cfm?doid=363011.363155</a><br/>
  Belady, Nelson,
  Shedler (1969); Communications of the *acm*

- /Understanding the Linux Virtual Memory Manager/
  #+latex:  \otrorec{http://ptgmedia.pearsoncmg.com/images/0131453483/downloads/gorman_book.pdf}
  #+html: <br/><a href="http://ptgmedia.pearsoncmg.com/images/0131453483/downloads/gorman_book.pdf">http://ptgmedia.pearsoncmg.com/images/0131453483/downloads/gorman_book.pdf</a><br/>
  Mel Gorman (2004).  Libro de libre descarga y redistribución, parte
  de la colección /Bruce Perens' Open Source Series/.  Aborda a
...
- /The art of picking Intel registers/
  #+latex: \otrorec{http://www.swansontec.com/sregisters.html}
  #+html: <br/><a href="http://www.swansontec.com/sregisters.html">http://www.swansontec.com/sregisters.html</a><br/>
  William Swanson (2003).

- /The Tao of Buffer Overflows/
  #+latex: \otrorec{http://sistop.gwolf.org/biblio/The_Tao_of_Buffer_Overflows_-_Enrique_Sanchez.pdf}
  #+html: <br/><a href="http://sistop.gwolf.org/biblio/The_Tao_of_Buffer_Overflows_-_Enrique_Sanchez.pdf">http://sistop.gwolf.org/biblio/The_Tao_of_Buffer_Overflows_-_Enrique_Sanchez.pdf</a><br/>
  Enrique Sánchez (trabajo en proceso, facilitado por el autor)

- /Smashing the Stack for fun and profit/
  #+latex: \otrorec{http://www.phrack.org/issues.html?issue=49&id=14}
  #+html: <br/><a href="http://www.phrack.org/issues.html?issue=49&id=14">http://www.phrack.org/issues.html?issue=49&id=14</a><br/>
  /Aleph One/ (1996): Uno de los primeros artículos publicados acerca de los
  /buffers desbordados/

- /Attacking the Windows 7/8 Address Space Randomization/
  #+latex: \otrorec{http://kingcope.wordpress.com/2013/01/24/}
  #+html: <br/><a href="http://kingcope.wordpress.com/2013/01/24/">http://kingcope.wordpress.com/2013/01/24/</a><br/>
  /Kingcopes/ (2013):
...
- /An overview of Non-Uniform Memory Access/
  #+latex: \otrorec{https://dl.acm.org/citation.cfm?doid=2500468.2500477}
  #+html: <br/><a href="https://dl.acm.org/citation.cfm?doid=2500468.2500477">https://dl.acm.org/citation.cfm?doid=2500468.2500477</a><br/>
