...
y /escribiendo/ al nuevo hasta llegar al fin de archivo fuente).
...
- Ubicación en disco :: El sistema mantiene esta infromación 
...
a un usuario a ejecutar un archivo ejecutable /disfrazado/ de un
...
- Acceso aleatorio :: El empleo de gestores como /SQLite/ u otros
     ...
     el archivo al cual apunta el descriptor de archivo =FD= hay 2000
     ...
     4~875$ (=seek(FD, 4875)=) y leer los siguientes 75 bytes en
...
- Archivo estándar :: Aunque suene obvio, el tipo básico de archivo
     es, precisamente, el archivo. Representa directamente a la
...
- Dispositivos :: Permiten el acceso directo a un dispositivo
                  ...
                  números, el /número mayor/ y el /número menor/;
                  tradicionalmente, el mayor indica de qué clase de
                  dispositivo se trata, y el menor, la instancia de
                  que se trata.

		  Por ejemplo, tanto los discos duros como sus
                  particiones llevan por número mayor el 8; los
                  números menores para los duros son 0, 16,
                  32, 48. Cada uno de los discos puede tener hasta 15
                  particiones: 1 a 15, 17 a 31, etc.
...
El concepto dominante en almacenaimiento hoy en día es el de /directorios
jerárquicos/. Esto no siempre fue así; conviene revisar brevemente su
...
directorios fue introducido apenas en la versión 2, junto con el
soporte a discos duros de 10 *mb*, acompañando al lanzamiento de la *ibm
pc* modelo *xt*.]
...
la definición de /archivos/ que se ha venido manejando) y /cubetas/ (de cierto modo
...
- Liga o enlace duro :: La entrada de un archivo en un directorio Unix es la
...
al archivo destino, sino que al acceso directo mismo.
...
- Listado de archivos :: Para mostrar los archivos que conforman a un
     ...
     =open()=), y va /leyendo/ (con =readdir()=) cada una de sus entradas.
     Cada uno de los resultados es una estrcutura =dirent=
...
esta discusión bastará saber que es un número único por volumen.]
...
acceso. Los permisos indican al sistema qué puede hacerse y qué no con
cada uno de los objetos, posiblemente diferenciando el acceso
dependiendo del usuario o clase de usuarios de que se trate.
...
Allocation Table/.] fue diseñado a fines de los 1970, y al día de hoy
es muy probablemente el sistema de archivos más ampliamente utilizado
del mundo. Su diseño es suficientemente simple para ser incluído en
...
- Oculto :: Si está en cendido, el archivo no se deberá mostrar al
...
/lectura/, /eescritura/ y /ejecución/. Estos permisos son los mismos
...
el permiso de lectura indica qué usuarios pueden averiguar la lista de
...
El esquema de persmisos de Unix presenta algunas características
...
necesario crear conjuntos de grupos adicionales que intersectan
aspectos específicos: una salida muy poco elegante.

#+caption: Lista de control de acceso a un archivo en Windows 7: En esta vista, muestra que el grupo =SYSTEM= tiene permiso para todo tipo de acceso a excepción del definido por /Permisos especiales/.
#+label: DIR_ACLs_Win7
...
En segundo término, resulta más dificil presentar un listado compacto
...
En tercer lugar, este esquema da lugar a ambigüedades, cuya política
...
explícita/ para la escritura a cierto archivo, y otro de ellos con
/denegación explícita/ para la escritura al mismo, debe haber una
política global para resolver si el permiso será otorgado o
rechazado. Estas políticas no siempre resultan fáciles de comprender
...
a las prestaciones relativas de una computadora, sino al papel que ésta
...
El sistema de archivos que ofrece sobre la red *nfs* cumple con la semántica Unix
...
El *nfs* viaja sobre un protocolo muy ligero — no implementa cifrado
ni verificaciones adicionales, pero al día de hoy, es uno de los
mejores mecanismos para el envío de grandes cantidades de información
— pero siempre en redes que sean /completamente confiables/.
...
el mismo usuario tiene diferentes identificadores, los permisos no
...
#+latex: \emph{Net\textbf{beui}},
#+html: <i>Net<b>beui</b></i>,
...
dado el lapso de tiempo que ha existido, ha pasado por varios cambios
...
usuario/contraseña, que posibilita el otorgamiento de permisos con una
...
que era fácil llegar a la saturación, y no existía un esquema
centralizado de resolución de nombres, con lo que era frecuente /no
...
** Sistemas de archivos distribuídos: Andrew File System
...
almacenamiento como equipos con papel de servidor. *afs* permite
...
estados van ligados al /momento en el tiempo/ en que se presentan, a
través de un /modelo de consistencia débil/ (/weak consistency
...
4. De las siguientes afirmaciones, identifique cuáles se refieren a
   /ligas duras/, cuáles a /ligas simbólicas/, y cuáles a /enlaces
   directos/. Hay una sóla respuesta correcta para cada afirmación.
...
6. Suponga que existe un archivo =file0= en un sistema de archivos con
   semántica tipo Unix. Explique qué ocurre en cada situación (con su
   inodo, con sus sectores de contenido, entradas de directorio, etc):
...
- Sistemas de archivos distribuídos ::
  ...
  /distribuído/: Bajo *afs*, los datos almacenados no están en una única
  ...
  de archivos verdaderamente distribuídos, aptos para la
  ...
  de desconexión o falla, parcial o total? ¿Qué características hacen
  a estos sistemas /especialmente/ buenos para su uso distribuído?
  ¿Habría alguna desventaja de emplear esos sistemas en un entorno de
  red local?

** Lecturas relacionadas
- /File System Interface: Functions for manipulating files/
  #+latex:  \otrorec{https://www.gnu.org/software/libc/manual/html_node/File-System-Interface.html}
  #+html: <br/><a href="https://www.gnu.org/software/libc/manual/html_node/File-System-Interface.html">https://www.gnu.org/software/libc/manual/html_node/File-System-Interface.html</a><br/>
...
- /Serverless network file systems/
  #+latex: \otrorec{https://dl.acm.org/citation.cfm?doid=225535.225537}
  #+html: <br/><a href="https://dl.acm.org/citation.cfm?doid=225535.225537">https://dl.acm.org/citation.cfm?doid=225535.225537</a><br/>
  Thomas
...
- /Finding open files with lsof/
  #+latex: \otrorec{http://www.ibm.com/developerworks/aix/library/au-lsof.html)}
  #+html: <br/><a href="http://www.ibm.com/developerworks/aix/library/au-lsof.html)">http://www.ibm.com/developerworks/aix/library/au-lsof.html)</a><br/>
