...
# Probablemente este capítulo podría partirse en sus dos componentes
# principales (¿dónde vi que lo hacían así?): Por un lado, la
# representación lógica de los archivos, y por otro, la representación
# de esta información en el disco.
...
#+label: FS_capas_acceso_a_discos
...
–aunque esta nomenclatura es a veces incorrecta– como /discos/.

Del diagrama presentado en la figura \ref{FS_capas_acceso_a_discos},
...
En esta unidad tocaremos también algunos conceptos relacionados con
los subsistemas de entrada y salida, aunque estos pertenecen
mayormente a otras unidades, al igual que los controladores.
...
los detalles de esta interfaz en la siguiente sección.
...
\ref{PROC} (/Administración de procesos/), los mecanismos que emplean
...
- Acceso secuencial :: Mantiene la semántica por medio de la cual
     ...
     \ref{FS_operaciones_con_archivos} (/Operaciones con archivos/):
...
#+attr_latex: width=\textwidth
#+label: FS_acceso_relativo_a_indice
...
En un grafo dirigido como el presentado en la figura \ref{FS_directorio_DAG}, un mismo nodo puede tener varios directorios
...
- Liga dura :: La entrada de un archivo en un directorio Unix es la
  ...
  2. No pueden hacerse ligas duras a directorios, sólo a archivos[fn::
     ...
     crearlas; detallaremos el por qué al hablar de recorrer los
     directorios (sección \ref{FS_dir_traversal}).]
...
Ya con el conocimiento de las ligas, podemos reelaborar la figura
\ref{FS_directorio_DAG} apegándonos más a la realidad: En los sistemas
operativos (tanto Unix como Windows), todo directorio tiene dos
entradas especiales: Los directorios =.= y =..=

#+attr_latex: width=0.5\textwidth
#+label: FS_directorio_DAG_completo
#+caption: Directorio como un /grafo dirigido/, mostrando los /enlaces ocultos/ =.= y =..=
[[./img/dot/directorio_DAG_completo.png]]

Como podemos ver en la figura \ref{FS_directorio_DAG_completo}, en
...
el sistema de archivos. Más adelante, en la sección
\ref{FS_medio_fisico}, abordaremos algunos de los aspectos que debemos
considerar al hablar de sistemas de archivos respaldados en medios
distintos a un disco.
...
lógico. Veremos más acerca de qué factores determinan su tamaño de
bloques cada sistema de archivos en la sección \ref{FS_el_directorio},
al hablar de el directorio.
...
en las tablas de asignación,[fn:: Abordaremos este tema en breve, en
la sección \ref{FS_tablas_fat}, al hablar de las tablas de asignación
de archivos] son eliminadas — Sólo son marcadas como /disponibles/. El
espacio de almacenamiento que el archivo eliminado ocupa debe,
entonces, ser /sumado/ al espacio libre que tiene el volumen. Es
apenas cuando se crea un nuevo archivo empleando esa misma entrada que
el sistema operativo marca /realmente/ como desocupados los clusters
en la tabla de asignación[fn:: Pero, claro, estos clusters podrían
mantenerse como ocupados /por el nuevo archivo/, ahorrando el trabajo
de volver a asignarlos.].
...
patrones comunes. Por ejemplo, dentro del directorio de trabajo de
cada uno de los usuarios hay típicamente archivos creados con los
mismos programas, compartiendo encabezado, estructura, y
ocasionalmente incluso parte importante del contenido. Dentro de los
directorios de binarios del sistema, habrá muchos archivos ejecutables
compartiendo el mismo /formato binario/.
...
sin embargo, con contenido poco predecible o con muy baja redundancia
(como la mayor parte de formatos de imágenes o audio, que incluyen ya
una fase de compresión, o empleando cualquier esquema de cifrado) la
compresión no ayuda, y sí reduce el rendimiento global del sistema en
que es empleada.

*** Compresión de volumen completo

El primer sistema de archivos que se popularizó fue /Stacker/,
comercializado a partir de 1980 por /Stac Electronics/. Stacker
...
Ahora, conjuntando la variabilidad derivada del enfoque probabilístico
al uso del espacio con el ubicarse como una compresión orientada al
volumen entero, resulta natural encontrar una de las complejidades
...
bloques de dimensiones regulares (hemos manejado, por ejemplo, el
tamaño típico de sector de 512 bytes), al poder estos traducirse o más
...
también comprimidas. Estas tablas, como hemos ya expuesto (y pronto
reiteraremos) contienen la información fundamental del sistema de
...
similar a la de Stacker. Microsoft mantuvo soporte para DoubleSpace
por siete años, cubriendo las últimas versiones de MS-DOS y las de
Windows 95, 98 y Millenium, pero como ya mencionamos, la compresión de
...
espacio ya ocupado, peroagrega complejidad al proceso (y abona en
contra de la compresión).
...
los registros no estarán ya estrictamente alineados al tamaño que el
...
ellas con el mismo sistema operativo, los mismos archivos se repiten
...
que mencionamos en la sección \ref{FS_grafo_dirigido} es que, en caso
...
cambios y evitará que esto afecte a los demás archivos.
...
probabilidad de colisión de $1 \over 2^{256}$]; este mecanismo permite
una búsqueda de coincidencias y confiables.
...
llegan al sistema, o, dado que es una tarea intensiva en uso de
procesador, realizarla como una tarea posterior de mantenimiento, en
un momento de menor ocupación del sistema.
...
desduplicación es ZFS[fn:: Presentaremos las características básicas
de ZFS en la sección \ref{FS_zfs}], desarrollado por Sun Microsystems
(hoy Oracle). En Linux, esta característica forma parte de BTRFS,
aunque no ha alcanzado los niveles de estabilidad como para
...
apareció a nivel de archivo, implementada en espacio de usuario (y con
operación orientada a archivos), como una de las características del
servidor de correo /Exchange Server/ entre los años 2000 y 2010. A
partir de Windows Server 2003, la funcionalidad de desduplicación
existe a nivel sistema de archivos para NTFS, pero su uso es poco
frecuente.
...
5GB de memoria por cada TB de almacenamiento]].
...
- Con el bloque de doble indirección, podemos referirnos a archivos
  mucho más grandes:

  $(100-3 + 128 + (128\times 128) ) \times 4KB = 66436KB \approx 65MB$

  Sin embargo, aquí ya debe llamarnos la atención otro importante
  punto: Si queremos acceder a estos 65MB, tendremos que realizar
  hasta 131 accesos a disco. A partir de este punto, el sistema
...
- Empleando triple indirección, llegamos hasta:
  ...
  bloques que tendríamos que leer, sin embargo, para encontrar todos
  los clusters nos significarían hasta 16516 accesos a disco.
...
suerte de archivos estructurados (ver sección
\ref{FS_archivos_estructurados}), gestionados por el sistema
...
gestión física directa de los dispositivos; en la sección
\ref{FS_medio_fisico} (/El medio físico/) abordaremos más a profundidad
lo que esto ha significado para el desarrollo de sistemas de archivos
y algoritmos relacionados. Sin embargo, en este tema, los
...
\ref{FS_estado_solido} abordaremos algunas de sus características), y
dado que estos sistemas aprovechan mejor varias de sus
características, es probable que el interés en estos sistemas de
archivos resurja.
...
#+attr_latex: width=0.9\textwidth
#+label: FS_mov_cabeza_por_algoritmo
...
- Familia de algoritmos /de elevador/ (SCAN, LOOK, C-SCAN) :: Hablamos
     ...
     del cilindro más bajo, brincará de vuelta hasta el más alto para
...
En la sección \ref{FS_estructura_en_dispositivo} (/Plasmando la
estructura en el dispositivo/) presentamos muy escuetamente al
...
- [[http://compression.graphicon.ru/download/articles/huff/huffman_1952_minimum-redundancy-codes.pdf][A method for the construction of Minimum-Redundancy Codes]] (Davud
  A. Huffman, 1952)
...
- [[http://data.openplanetsfoundation.org/ref/][OpenPlanets Results Evaluation Framework]], Open Planets
  ...
  reconocen archivos de tipos conocidosvarias herramientas (para
  reconocimiento de tipo de archivos)
