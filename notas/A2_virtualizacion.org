...
fuera de los participantes de dicho programa de licenciamiento.
...
verdadero aislamiento, puede referirse al artículo /[[http://www.bpfh.net/computing/docs/chroot-break.html][How to break out
...
1. Explique ventajas y desventajas de los diferentes tipos de
   virtualización (hipervisores y emuladores).
...
2. A continuación se presentan varias afirmaciones. Evalúe si cada una
   ...
   - La emulación implica naturalmente el implementar un intérprete
     del código que fue originado para una arquitectura distinta. Este
     ...
     ejecución nativa. La popularización de la emulación deriva por un
     lado de la diferencia de poder de cómputo entre las arquitecturas
     de escritorio y las embebidas, y por el otro de la conveniencia
     de contar con código que pueda ser transportado fácilmente
   ...
   - La /Ley de Moore/ (presentada en la sección
     ...
     desarrolladores de software; casi todo el material de la presente
     obra deriva del cuestionamiento de cómo compartir recursos entre
     procesos rivales.
...
4. *Agregar conceptos*
   ...
   Los contenedores son un enfoque muy distinto que, sin embargo,
   logra un resultado bastante similar al de las otras tecnologías de
   virtualización. Para los conceptos, /chroot/ y /contenedores o
   contextos de seguridad/, elija de la siguiente lista cuál
   definición corresponde a cada uno.
...
5. La principal diferencia entre la virtualización asistida por
   hardware (HVM) y la paravirtualización es que...

   1. La virtualización asistida por hardware requiere de hardware
      específico para ser empleada, la paravirtualización intercepta
      por software las instrucciones "peligrosas", a cambio de un
      rendimiento un poco menor (~30%)

   2. La virtualización permite ejecutar al sistema huésped sin
      modificaciones, mientras que la paravirtualización requiere que
      éste esté preparado para ser virtualizado

   3. Se efectúa únicamente a nivel de dispositivos, mientras que la
      virtualización plena se aplica sobre del sistema completo

6. En el corazón de las arquitecturas de virtualización por hardware
   encontraremos un programa llamado hipervisor. Indique cuáles de las
   siguientes afirmaciones respecto a éste son verdaderas, y cuáles
   falsas.
   ...
   3. Es completamente transparente: Un hipervisor debe poder correr
      otros hipervisores dentro de sí
...
- Sistemas operativos mínimos para la nube ::
  ...
  tradicional, sino que únicamente permitir la ejecución de varios de
  los procesos más comunes en una máquina virtual. Pueden buscar al
  respecto:
