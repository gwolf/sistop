...
#+caption: Principales formatos de disco flexible que lograron popularizarse en el mercado.
|-----------------------+---------------------+---------------------+----------------------|
...
duros son metálicos. Los discos duros están /permanentemente/
...
- *sstf* :: Ahora bien, si el factor que impone la principal demora es
	  ...
	  Una desventaja de *sstf* es que puede llevar a la inanición:
          si hay una gran densidad de solicitudes para
          determinada zona del disco, una solicitud para un cilindro
          alejado puede quedar a la espera indefinidamente.
	  ...
	  Ejemplificando esto con una serie de solicitudes distinta a
          ...
          cilindro 175 puede quedar en espera de forma indefinida.
...
- Memoria /flash/ :: Derivada de los *eeprom* (/Electrically Erasable
  ...
  - Transporte de archivos :: Esta tecnología también está presente en
       ...
       *usb*, *sd*, /Memory Stick/, /Compact Flash/, etc. La principal
       diferencia entre éstas son los diferentes conectores que
...
la ilustra \textcite{Aurora2009}, que menciona que
...
Aurora indica que hay varias operaciones que no pueden ser traducidas
eficientemente por medio de esta capa de emulación, y que seguramente
permitirían un mucho mejor aprovechamiento del medio. Como se mencionó
en la sección \ref{FS_log_structured} (/Sistemas de archivo
estructurados en bitácora/), si bien varios de estos sistemas de
archivos han presentado implementaciones completamente utilizables, la
falta de interés ha llevado a que muchos de estos proyectos sean
abandonados.

\textcite{Brown2012} apunta a que Linux tiene una interfaz apta para
manipular directamente dispositivos de estado sólido, llamada =mtd= —
...
La velocidad de acceso a los datos bajo *raid* 1 es menor a la que se
lograría con un disco tradicional: no basta con obtener los datos de
uno de los discos; el controlador *raid* (sea el sistema operativo o
una implementación en hardware) recupera ambas copias, y sólo las
entrega a los programas si concuerdan; en caso contrario, debe
reportar un error en el arreglo. La velocidad de escritura se ve
levemente reducida, dado que hay que esperar a que ambos discos
escriban la información.
...
#+attr_latex: width=0.8\textwidth placement=[b]
#+label: FS_FIS_raid_5
